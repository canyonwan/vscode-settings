<script setup lang="ts">
  ...
  const { discretionList } = inject<IDiscretionOptions>(discretionOptionDataKey)!
  ...
  watch(
    ...
    async () => {
      await injectReloadDiscretionDataForJunior()
      console.log('%c [ d-type discretionList ]-119', 'font-size:13px; background:#0095FF; color:white;', discretionList)
    },
    {
      immediate: true,
      deep: true
  ...
  watchEffect(() => {
    if (discretionList.length > 0) {
      // 查询当前选中的裁量类型
      // const res = injectDiscretionOptions.discretionList.find((item) => item.isChecked === 1)
      const res = discretionList.find((item) => item.id === discretionBaseState.discretionBaseId)
      if (res) {
        res.isChecked = 1
  ...
  watchOnce(
    () => discretionList,
    () => {
      const res = discretionList.find((item) => item.isChecked === 1)
      if (res) {
        currentSelectDiscretionItem.value = res
        discretionBaseState.baseValue = res.baseValue
        discretionBaseState.discretionBaseId = res.id
...
<template>
  <div class="discretion-type bg-contentBgColor" v-if="discretionList?.length > 0">
    <n-space vertical>
      <n-radio-group :value="discretionBaseState.discretionBaseId" @update:value="onRadioGroupChanged" name="radiogroup">
        <n-space vertical v-for="item in discretionList" :key="item.id" class="my-4">
