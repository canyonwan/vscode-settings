        ...
        <n-space vertical v-for="item in injectDiscretionOptions.discretionList" :key="item.id" class="my-4">
          <n-space justify="space-between">
            <n-radio :value="item.id">
              <span class="font-medium">{{ `${discretionTypeOptions.get(item.discretionTypeId)} ${item.baseKey ? `（${item.baseKey}）` : ''} ` }}</span>
            </n-radio>
            ...
            <n-space class="pl-1 text-gray-400 text-xs leading-5">
              ...
              <n-input-number
                v-if="item.discretionTypeId !== '1000000' && discretionBaseState.discretionBaseId === item.id"
          ...
          <!-- 证据列表 -->
          <div v-if="discretionBaseState.discretionBaseId === item.id">
