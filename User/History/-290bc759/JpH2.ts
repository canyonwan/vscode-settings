...
import type { RenderableComponent } from './typs'
...
export const UseDraggable = defineComponent<UseDraggableProps>({
  name: 'UseDraggable',
  props: ['storageKey', 'storageType', 'initialValue', 'exact', 'preventDefault', 'pointerTypes', 'as'] as unknown as undefined,
  setup(props, { slots }) {
    const target = ref()
    const initialValue = props.storageKey
      ? useStorage(props.storageKey, unref(props.initialValue) || { x: 0, y: 0 }, isClient ? (props.storageType === 'session' ? sessionStorage : localStorage) : undefined)
