<script setup lang="ts">
  ...
  function onUpdateValue(causeIds: string[]) {
    console.log('%c [ causeIds ]-12', 'font-size:13px; background:pink; color:#bf2c9f;', causeIds)
    ...
    //     causeContainer.push({
    //       causeId: res,
    //       caseTypeId: ele.caseTypeId
    //     })
    //   }
    // }
    // emits('handleUpdateValue', causeContainer)

    console.log('%c [  injectCheckedChildren]-26', 'font-size:13px; background:pink; color:#bf2c9f;', injectCheckedChildren?.value)
  }
</script>
<template>
  <n-checkbox-group :value="injectCheckedChildren" @update:value="onUpdateValue">
    <n-space vertical>
      <n-checkbox v-for="node in injectNodeRubbish" :key="`${node.causeId}&${node.caseTypeId}`" :value="`${node.causeId}&${node.caseTypeId}`">
