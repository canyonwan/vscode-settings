<script setup lang="ts">
  ...
  function onUpdateValue(causeIds: string[]) {
    ...
    //     if (causeId === item.causeId) return item.punishId
    //     // else console.log('else');
    //   }
    // })
    for (const item of injectNodeRubbish!.value) {
      let punishIds = causeIds.map((causeId) => {
        if (causeId === item.causeId) {
          return item.punishId
        }
        // return causeId === item.causeId
      })
    }
    console.log('update value causeIds', causeIds)
    console.log('update value punishIds', punishIds)
