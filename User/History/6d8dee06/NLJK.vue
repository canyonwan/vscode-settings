<script setup lang="ts">
  ...
  function onUpdateValue(causeIds: string[]) {
    ...
    console.log('%c [ 违法行为 ]-12', 'font-size:13px; background:pink; color:#bf2c9f;', injectCheckedChildren?.value)
    ...
    //   }
    // }
    emits('handleUpdateValue', causeIds)
  }
</script>
<template>
  <n-checkbox-group :value="injectCheckedChildren" @update:value="onUpdateValue">
    <n-space vertical>
      <n-checkbox v-for="node in injectNodeRubbish" :key="`${node.causeId}&${node.caseTypeId}`" :value="`${node.causeId}&${node.caseTypeId}`">
        ...
        <n-card>
          <div class="font-medium text-base">{{ node.causeName }}（{{ node.caseType }}）</div>
