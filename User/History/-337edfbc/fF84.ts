  ...
  if (schema && schema.length) {
    ...
    if (
      ...
      for (const item of schema) {
        ...
        } else if (item.componentDataSourceType === 'EnumsType' && item.componentDataSourceName === 'litigation') {
          // 诉讼枚举
          item.componentProps.options = enumsTypeData.filter((info) => {
            console.log('%c [ info ]-319', 'font-size:13px; background:#0095FF; color:white;', info)
            if (item.componentDataSourceName === info.pojoEntityName) {
              info.label = info.enumChineseDescription
              info.value = info.enumCode
              return info
