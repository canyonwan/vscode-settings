...
const schemaDataHandle = async (schema: FormSchema[]) => {
  if (schema && schema.length) {
    ...
    let causeTypeData: ICauseType[] = []
    let caseTypeData: ICaseType[] = []
    ...
    if (
      ...
      for (const item of schema) {
        if (item.componentDataSourceType === 'Dictionaries' && item.componentDataSourceName) {
          ...
          console.log('%c [ item.componentDataSourceEnumPrefix ]-310', 'font-size:13px; background:#0095FF; color:white;', item.componentDataSourceEnumPrefix)
          item.componentProps.options = enumsTypeData.filter((info) => {
            console.log('%c [  info.enumDescription.split('_')[0] ]-311', 'font-size:13px; background:#0095FF; color:white;',  info.enumDescription.split('_')[0])
            if (item.componentDataSourceName === info.pojoEntityName && info.enumDescription.split('_')[0] === item.componentDataSourceEnumPrefix) {
              ...
              return info
            } else if (item.componentDataSourceName === info.pojoEntityName) {
              info.label = info.enumChineseDescription
              info.value = info.enumCode
              return info
            }
