  ...
  <div class="personnel-add-edit">
    <basic-form @register="register" @submit="handleSubmit">
      ...
      <template #customForm="{ model, disabled }">
        <custom-form-use :type="2" ref="customFomRef" :customFormValues="model.customFormValues" :disabled="disabled" />
      </template>
    </basic-form>
  </div>
</template>
    ...
    setup(props, context) {
        ...
        () => {
          if (props.personnelId) {
            ...
            nextTick(() => {
              ...
              console.log('%c [ state.formParams ]-106', 'font-size:13px; background:#0095FF; color:white;', state.formParams)
            })
          }
        },
        {
      ...
      watch(
        ...
        () => {
          ...
          nextTick(() => {
            setFieldsValue(state.formParams)
            console.log('%c [orgId state.formParams ]-120', 'font-size:13px; background:#0095FF; color:white;', state.formParams)
          })
        },
        {
          immediate: true
      ...
      const handleSubmit = async () => {
        ...
        if (res.enforceFile.length > 0) {
          res.enforceFile = res.enforceFile.map((file) => file.fileId)[0]
        }
