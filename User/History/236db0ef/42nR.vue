<script setup lang="ts">
  import { UseDraggable as Draggle } from '@vueuse/core'
...
<template>
  <div class="case-detail">
    ...
    <n-space justify="space-between">
      <n-button class="mb-4" type="primary" @click="$router.back()">返回</n-button>
      <n-switch :round="false" :value="showStageBar" @update:value="switchDisplayMode" v-show="programTypeId === 2">
        <template #checked> 展示阶段 </template>
        <template #unchecked> 展示全部 </template>
      </n-switch>
    </n-space>
    <n-grid cols="1 s:1 m:1 l:24 xl:24 2xl:24" responsive="screen" :x-gap="12">
      <!-- 关联程序bar -->
    ...
    <component :is="Draggle" :initialValue="{ top: '50%', right: 0 }" :preventDefault="true" :stopPropagation="true">
      <div v-if="isEnforcer" class="fixed z-50 animate-pulse cursor-move bg-e-tools-box-icon bg-no-repeat bg-full h-20 w-20" ref="toolsBoxRef" :style="style" @click="openModal()"></div>
    </component>
