...
import { fineColumns, IAction } from './options'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const ac = ref(actions)

    function processData() {
      console.log('%c [ before ac ]-107', 'font-size:13px; background:#0095FF; color:white;', ac.value)
      actionsTemp.value.length = 0
      for (const i of listDataForType.value) {
        if (i.count > 0) {
          actionsTemp.value.push(ac.value[i.type - 1])
        }
      }
      console.log('%c [ after ac ]-107', 'font-size:13px; background:#0095FF; color:white;', ac.value)
    }
