    ...
    <a-spin :spinning="loading">
      <a-form ref="formRef" :model="formState" :rules="rules" :labelCol="{ flex: '120px' }" :wrapperCol="{ flex: '1' }">
        ...
        <a-form-item
          ...
          name="handleDateList"
        >
          <a-range-picker v-model:value="formState.handleDateList" style="width: 220px" allowClear />
        </a-form-item>
        <a-form-item
          label="设定催办时间"
          v-if="matterType === 2"
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const fetchTopUnitList = async () => {
      const topUnit = await getOrgData({ orgId4BelongToJusticeId: user.unitOrgId })
    ...
    const onSubmit = () => {
      formRef.value.validate().then(() => {
        ...
        // 办理期限
        // params.beginDate = internalInstance?.appContext.config.globalProperties.$parseTime(formState.handleDateList[0], '{y}-{m}-{d} 00:00:00')
        // params.endDate = internalInstance?.appContext.config.globalProperties.$parseTime(formState.handleDateList[1], '{y}-{m}-{d} 23:59:59')
        params.startDate = dayjs(state.timeR[0]).format('YYYY-MM-DD 00:00:00')
        params.endDate = dayjs(state.timeR[1]).format('YYYY-MM-DD 23:59:59')
        // 催办时间
        params.remindDate = internalInstance?.appContext.config.globalProperties.$parseTime(params.remindDate, '{y}-{m}-{d} 00:00:00')
        state.loading = true
