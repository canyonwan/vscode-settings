...
import { querySignUpRecord, SignUpRecordItemType, TimeRangeValueType } from '@/api/subject/law-test'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const state = reactive({
      ...
      signUpDateList: [] as any as TimeRangeValueType,
    ...
    const getData = () => {
      ...
      if (state.signUpDateList) {
        params.signUpStartDate = dayjs(state.signUpDateList[0]).format('YYYY-MM-DD')
        params.signUpEndDate = dayjs(state.signUpDateList[1]).format('YYYY-MM-DD')
      }
