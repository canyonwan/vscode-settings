...
<template>
  <n-card class="discretion" ref="el">
    ...
    <div v-else>
      <n-tabs class="card-tabs" size="large" animated style="margin: 0 -4px" pane-style="padding-left: 4px; padding-right: 4px; box-sizing: border-box;">
        <n-tab-pane v-for="item in causeListData" :key="item.id" :name="`${item.id}+${Math.round(Math.random())}`" :tab="item.name">
