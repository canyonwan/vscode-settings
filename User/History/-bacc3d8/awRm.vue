<script setup lang="ts">
  import { queryCauseListForProgram, queryDiscretionOptionData } from '@/api/program/program'
...
import DTabPane from './components/d-tab-pane.vue'
  ...
  async function getCauseListForProgram() {
    ...
    await getDiscretionOptionsData(causeListData.value.)
  }

  // 获取裁量选项数据
  async function getDiscretionOptionsData(id: string) {
    const res = await queryDiscretionOptionData(id)
  }
  ...
  <n-card class="discretion" ref="el">
    <n-tabs v-for="item in causeListData" :key="item.id" class="card-tabs" size="large" animated style="margin: 0 -4px" pane-style="padding-left: 4px; padding-right: 4px; box-sizing: border-box;">
      <n-tab-pane :name="item.id" :tab="item.name">
        <d-tab-pane :current-step="currentStep" @handleNextStep="handleNextStep" />
      </n-tab-pane>
    </n-tabs>
  </n-card>
</template>
