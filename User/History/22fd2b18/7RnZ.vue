    ...
    <div class="content">
      <n-grid :cols="Object.keys(caseInfo).length ? 8 : 20" :x-gap="48" :style="{ height: 'calc(100vh - ' + 185 + 'px)' }">
        ...
        <n-gi v-if="!Object.keys(caseInfo).length" :span="2">
          <div class="back_bg">
            <div class="cursor-pointer back_button_bg bg-blue-50 bg-opacity-10" type="primary" @click="onBackToTimeline">返回<br />记录轴</div>
          </div>
        </n-gi>
      </n-grid>
    </div>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props, context) {
      ...
      const bus = useEventBus('onBackToTimeline')
      function onBackToTimeline() {
        router.back()
        bus.emit('1')
      }
