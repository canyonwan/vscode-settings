<script setup lang="ts">
  ...
  import { announcementColumn, announcementInnerBoxColumn } from '@/views/inform/announcement/columns'
  ...
  async function viewDetail(row: IMessageItem) {
    setProps({ title: '查看详情' })
    if (queryType.value === 2 && row.whetherRead === 0) {
  ...
  function onDelete() {
    dialog.error({
      ...
      onPositiveClick: async () => {
        await deleteAnnouncement(selectedIds.value, queryType.value)
        reload()
      }
    })
  }
...
<template>
  <div class="announcement">
    <n-form inline :label-width="100" :model="searchParams">
    ...
    <basic-table
      ...
      :columns="queryType === 2 ? announcementInnerBoxColumn : announcementColumn"
    ...
    >
      ...
      <template #toolbar><n-button type="success" v-show="queryType === 3" @click="sendAnnouncement">发送公告</n-button></template>
    </basic-table>
