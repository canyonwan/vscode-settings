...
import { NButton, NButtonGroup, NCollapse, NCollapseItem, NDivider, NEmpty, NSpace, NTag, useDialog } from 'naive-ui'
import { Ref } from 'vue'
  ...
  setup(props: HandlingWayCommonPropsType) {
    ...
    return () => (
      <>
        ...
        {/* 列表 */}
        {state.handlingWays.length === 0 && <NEmpty style="min-height: 200px" class="justify-center" />}
        <NCollapse>
          {state.handlingWays.map((item, index) =>
            item.groupList.length > 0 ? (
              ...
              <>
                ...
                {index !== state.handlingWays.length - 1 && <NDivider />}
              </>
            )
          )}
        </NCollapse>
