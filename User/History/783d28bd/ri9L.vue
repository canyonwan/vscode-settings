...
export default defineComponent({
  ...
  setup(_props, context) {
    ...
    const getData = () => {
      // 保定市的机关数据
      state.loading = true
      getOrgData({ organizationType: 1, filter: 1 }).then(
        (data) => {
    ...
    const onSelect = (orgData: any) => {
      ...
      context.emit('onSelect', { orgId: state.currentOrgId, isSameLevel: isSameLevel, isSubordinate: isSubordinate, isJustice: isJustice, belongJusticeOrgId: orgData.belongJusticeOrgId })
    }
