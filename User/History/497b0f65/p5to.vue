<script setup lang="ts">
  ...
  import EvidenceLeaf from '@/views/program/components/vertical-case-detail/evidence-leaf.vue'
...
<template>
  <div class="timeline-item p-4 ml-12 mr-4 mb-6">
    <!-- 阶段按钮 -->
    <div class="e-button mb-6 flex items-center justify-between text-sm cursor-pointer" @click="handleClick(props.itemNode)">
      <div class="pr-4">
    ...
    <div class="flex">
      <div class="flex flex-col px-2" v-for="(timelineNode, idx) in props.itemNode.childrenList" :key="timelineNode.id">
        <!-- Evidence node toolType: 8 -->
        <!-- <evidence-leaf v-if="timelineNode.toolType === 8" :children="timelineNode.childrenList" /> -->
        <div v-if="timelineNode.toolType === 8">
          {{ timelineNode.childrenList.length }}
        </div>

        <!-- other node -->
        <node-leaf v-else :item-node="timelineNode" :idx="idx" :parentChildrenLength="props.itemNode.childrenList.length" />
      </div>
    </div>
