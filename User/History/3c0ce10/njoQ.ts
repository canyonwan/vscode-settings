...
import type { IMessageItem, IMessageSearch, IMessageSend, IMessageDetail, IAnnouncementDetail } from './types'
 ...
 * 发送公告
 */
export function sendAnnouncement(params: IAnnouncementDetail): Promise<void> {
  return http.request(
    {
...
export function setRead(ids: string[]): Promise<number> {
  return http.request(
    {
      url: `${serverName}notice/setReadForJson`,
      method: RequestEnum.POST,
      data: { ids }
    },
    {
 ...
 * 批量删除公告
 */
export function deleteAnnouncement(ids: string[], queryType: number): Promise<number> {
  return http.request(
    {
      url: `${serverName}notice/deletePersonalNewsForJson`,
      method: RequestEnum.POST,
      data: { ids, queryType }
    },
    {
