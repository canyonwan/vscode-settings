  ...
  <div class="training-record">
    <a-row>
      <a-form-item label="培训年份">
        <a-date-picker picker="year" v-model:value="currentYear" @change="panelChangeOne" placeholder="请选择年份" format="YYYY" />
        <!-- @openChange="openChangeOne" @panelChange="panelChangeOne" :open="yearShowOne"  -->
      </a-form-item>
    </a-row>
    <a-table
      bordered
...
export default defineComponent({
  ...
  setup(props) {
    ...
    // 得到年份选择器的值
    const panelChangeOne = (value: string | dayjs.Dayjs, dateString: string) => {
      state.searchData.trainYear = value as string
    ...
    return { ...toRefs(state), openFile, user, currentYear, handleTableChange, panelChangeOne, onSendSignUpForm, onSignUpRecord }
  }
})
</script>
