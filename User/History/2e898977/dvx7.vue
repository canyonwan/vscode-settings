  ...
  <div class="training-record">
    <a-row>
      <a-form-item label="培训年份">
        <a-date-picker picker="year" v-model:value="currentYear" @change="onTimeChange" placeholder="请选择年份" format="YYYY" />
        <!-- @openChange="openChangeOne" @panelChange="onTimeChange" :open="yearShowOne"  -->
      </a-form-item>
    </a-row>
    <a-table
      bordered
...
export default defineComponent({
  ...
  setup(props) {
    ...
    // 得到年份选择器的值
    const onTimeChange = (value: string | dayjs.Dayjs, dateString: string) => {
      state.searchData.trainYear = dayjs(value).year().toString()
    ...
    const initData = () => {
      state.pagination.current = 1
      state.searchData.trainYear = currentYear.value.year().toString()
      getData()
    }
    onMounted(() => {
      initData()
    ...
    return { ...toRefs(state), openFile, user, currentYear, handleTableChange, onTimeChange, onSendSignUpForm, onSignUpRecord }
  }
})
</script>
