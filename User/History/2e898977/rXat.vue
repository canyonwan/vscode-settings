  ...
  <div class="training-record">
    <a-row>
      <a-form-item label="培训年份">
        <a-date-picker mode="year" v-model:value="searchData.trainYear" placeholder="请选择年份" format="YYYY" @openChange="openChangeOne" @panelChange="panelChangeOne" :open="yearShowOne" />
      </a-form-item>
    </a-row>
    <a-table
      bordered
...
import { createVNode, defineComponent, getCurrentInstance, onMounted, reactive, toRefs, watch } from 'vue'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    // 得到年份选择器的值
    const panelChangeOne = (value: string) => {
    ...
    const initData = () => {
      state.pagination.current = 1
      state.searchData.trainYear = dayjs(new Date()), '{y}')
      getData()
    }
    onMounted(() => {
      initData()
    ...
    return { ...toRefs(state), openFile, user, handleTableChange, openChangeOne, panelChangeOne, onSendSignUpForm, onSignUpRecord }
  }
})
</script>
