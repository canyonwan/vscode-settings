<template>
  <div class="collect-evidence">
    ...
    <basic-table :columns="columns" :action-column="!isSelect ? actionColumn : null" :request="resetInit" ref="tableRef" :row-key="(row: any) => row.id" :pagination="false" :isFixedHeight="true">
      <template #tableTitle>
    ...
    setup(props, context) {
      ...
      const handleAdd = () => {
        state.evidenceInfo = {
          ...
          causeId: null, // 违法行为ID causeRuleId 传了则违法行为ID也 必传
      ...
      const evidenceSubmit = async () => {
        ...
        if (formRes) {
          closeModal()
          if (props.isSelect) {
            const fileIds = formRes.map((item) => item.id) || []
            state.selectFileIds.push(...fileIds)
