<template>
  <div class="collect-evidence">
    ...
    >
      <template #tableTitle v-if="!isSelect">
  ...
  export default defineComponent({
    ...
    setup(props, context) {
      ...
      const resetInit = async () => {
        ...
        state.searchParam.proceduresId = props.caseInfo.procedureId
      ...
      const handleAdd = () => {
        state.evidenceInfo = {
          id: '', // 证据关联表ID
          evidenceId: '', // 证据ID
          proceduresId: props.caseInfo.procedureId, // 程序ID 必传
          ...
          relationType: props.caseInfo.relationType, // 关联类型 1.和程序关联 2.裁量规则 3.裁量类型证据
          name: '',
          proveMatter: '', // 证明事项
      ...
      const submitBindEvidenceFile = () => {
        const params: IBindEvidenceFile = {
          procedureId: props.caseInfo.procedureId.toString(),
