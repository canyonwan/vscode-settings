...
<script setup lang="ts">
  ...
  async function getProgramDetail() {
    ...
    if (!hideRelatedProgramBar.value) {
      findSameProgram = programDetail.value.relevanceList!.find((item) => item.id === (tansferProgramId.value || programDetail.value!.id))!
      timelineId.value = findSameProgram!.timeLineId
      programTypeId.value = findSameProgram?.procedureType
    } else {
      timelineId.value = programDetail.value.timeLineI
  ...
  async function jumpRelateProgram() {
    ...
    await updateProgramDetail()
    console.log('%c [ programDetail.value ]-247', 'font-size:13px; background:#0095FF; color:white;', findSameProgram.value)
