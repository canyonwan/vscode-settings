...
<script setup lang="ts">
  ...
  import ConfiscateDiscretion from '@/views/program/confiscate-discretion/index.vue'
  ...
  const [showConfiscateRegister, { openModal: openConfiscate }] = useModal({ title: '没收裁量', displayDirective: 'if' }) // 没收裁量
  ...
  async function handleClickShow(isToolBox: boolean, nodeToolInfo) {
    ...
    } else if (caseInfo.value.toolType === 15) {
      setRelateProgramProps({ title: caseInfo.value.title })
      if (isBox.value) openConfiscate()
        else openTaskTimeline()
    }
  }
  ...
  async function jumpRelateProgram() {
    tansferProgramId.value = caseInfo.value.toolType === 13 ? caseInfo.value.dataId : caseInfo.value.procedureId
    await updateProgramDetail()
...
<template>
  <div class="px-5 py-3 v-case-detail">
    ...
    <!-- 没收裁量 -->
    <BasicModal @register="showConfiscateRegister" style="width: 50%">
      <ConfiscateDiscretion :caseInfo="caseInfo" />
      <!-- <ShowEnforceCertificate :caseInfo="caseInfo" @submit-handler="submitAvoidHandler" /> -->
    </BasicModal>
    <!-- 检查对象详情 -->
