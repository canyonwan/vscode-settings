...
<template>
  <div class="home">
    ...
    <div class="content">
      ...
      <div class="pos">
        当前位置: 执法主页 >
        <template v-if="route.name === 'publicityAfterDetail'">
          <span @click="router.go(-1)">
            {{ route.name.includes('After') ? '事后公开 > ' : '事前公开 > ' }}
          </span>
          {{ route.meta.title }}
        </template>
        <template v-else>
      ...
      <div class="tabContent">
        <org-select v-if="route.name !== 'publicityAfterDetail'"></org-select>
        <router-view style="margin-left: 16px"></router-view>
      </div>
    </div>
  </div>
</template>
