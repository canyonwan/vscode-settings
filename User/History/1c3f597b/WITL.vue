...
export default defineComponent({
  components: { Descriptions, Descriptions.Item },
  setup() {
...
<template>
  <div class="other-detail">
    <Descriptions v-if="state" bordered :contentStyle="{ border: '1px solid #9accff' }" :labelStyle="{ border: '1px solid #9accff', backgroundColor: '#ebf8ff', width: '160px' }">
      <Descriptions.Item label="详情信息" :span="4" :contentStyle="{ backgroundColor: '#ebf8ff' }" :labelStyle="{ backgroundColor: '#ebf8ff', fontWeight: 'bold' }"></Descriptions.Item>
      <Descriptions.Item label="决定书(通知书)文号" :span="4" :labelStyle="{ fontSize: '18px', fontWeight: 'bold' }" :contentStyle="{ fontSize: '18px', fontWeight: 'bold' }">{{
      ...
      <Descriptions.Item v-if="isFine" label="全文公示" :span="4">
        <a v-for="(item, index) in state.fileList" :key="item" @click="onOpenFile(item)" style="color: #0084ff"> {{ `点击查看${index + 1}、 ` }} </a>
      </Descriptions.Item>
    </Descriptions>
  </div>
</template>
