...
export default defineComponent({
  ...
  setup() {
    ...
    async function getDetail() {
      ...
      if (res.data && res.data.fileList > 0) {
        hasFiles.value = true
      } else {
        hasFiles.value = false
      }

      // hasFiles.value = res.data?.fileList.length > 0
    }

    function onOpenFile() {
      const { href } = router.resolve({ path: 'imagesWall' })
      sessionStorage.setItem('files', JSON.stringify(state.value!.fileList))
  ...
  <div class="other-detail">
    <a-descritions v-if="state" bordered :contentStyle="{ border: '1px solid #9accff' }" :labelStyle="{ border: '1px solid #9accff', backgroundColor: '#ebf8ff', width: '160px' }">
      ...
      <a-descritions-item v-if="isFine" label="全文公示" :span="4">
        <a v-if="hasFiles" style="color: #0084ff" @click="onOpenFile">点击查看</a>
      </a-descritions-item>
    </a-descritions>
  </div>
</template>
