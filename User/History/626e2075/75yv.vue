...
<template>
  <div class="actions">
    <div v-for="(item, index) in props.data" :key="item.value" @click="onSwitch(item)" :class="['action', { isSelected: currentAction === item.value }]">
      <div class="icon-item">
        <img :src="item.icon" alt="" width="40" />
        <div>{{ item.text }}</div>
      </div>
      <div class="line" v-if="index !== props.data.length - 1"></div>
    </div>
  </div>
</template>
