...
<template>
  <div class="task-detail">
    <n-card>
      ...
      <!-- 文件 -->
      <media-preview class="ml-4" v-if="taskBaseInfo?.fileList" :values="taskBaseInfo?.fileList" />
      <div>办理期限：{{ taskBaseInfo && `${format(taskBaseInfo?.startTime, 'yyyy-MM-dd')}-${format(taskBaseInfo?.deadline, 'yyyy-MM-dd')}` }}</div>
    </n-card>
