...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      // 获取案由列表
      async function getCauseMangeList(searchParams: { name: string }) {
        let res: IResultPage<IPrimaryCauseItemType[]> = await queryCauseManageList({ ...searchParams, ...state.pageParams })
      ...
      function onDeleteSubCause(id: string) {
        dialog.error({
          ...
          onPositiveClick: async () => {
            ...
            await getCauseMangeList({ ...state.pageParams })
          }
        })
      }
      ...
      onMounted(() => {
        getCauseMangeList({ ...state.pageParams, ...state.searchParams })
        // queryCauseTags()
      })

      // 保存的回调刷新列表
      function submitHandler() {
        getCauseMangeList({ ...state.pageParams })
      }
