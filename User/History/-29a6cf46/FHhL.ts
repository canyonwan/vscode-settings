...
export const useGetAddress = async (aMap, mapInstance, lnglat: string[]): Promise<string> => {
    ...
    aMap.plugin('AMap.Geocoder', () => {
      ...
      if (lnglat.length > 0) {
        marker = new aMap.Marker({
          position: new aMap.LngLat(lnglat[1], lnglat[0]) // 经纬度对象，也可以是经纬度构成的一维数组[116.39, 39.9]
        })
