<template>
  <div class="main-header h-full">
    <div :class="[props?.options.length > 0 ? 'justify-between' : 'justify-end', 'flex']">
      <div v-if="props.options.length > 0">
        <div
          v-for="item in props.options"
          :key="item.value"
          class="inline-block transition duration-200 ease-linear cursor-pointer w-40 h-10 -mb-2.5 font-medium text-center text-mainHeader-normalText leading-10 rounded-tl-3xl bg-gradient-to-r from-mainHeader-gradientFrom to-mainHeader-gradientTo"
