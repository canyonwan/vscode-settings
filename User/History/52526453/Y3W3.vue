<script setup lang="ts">
  import { deleteAnnouncement, queryMessages, setRead } from '@/api/infrom'
  ...
  function onDelete() {
    dialog.error({
      ...
      onPositiveClick: async () => {
        await deleteAnnouncement(selectedIds.value.join(','))
        reloadTable()
      }
    })
  }
  ...
  async function onSetRead() {
    await setRead(selectedIds.value.join(','))
    reloadTable()
  }
...
<template>
  <div class="message">
    ...
    >
      <template #tableTitle>
        <n-button class="mr-4" type="warning" ghost @click="onSetRead" :disabled="selectedIds.length === 0" v-show="searchParams.whetherRead === 1">一键已读</n-button>
