    ...
    <a-card hoverable>
      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.dataIndex === 'caseName'">
          ...
          <template v-if="column.dataIndex === 'majorAdministrativePenaltyRecordStatus'">
            <a-badge
              :status="
  ...
  setup() {
      ...
      columns: [
        ...
        },
        {
          ...
          customCell: (record: any) => {
            return {
              innerHTML: dayjs(record.closingTime).format('YYYY-MM-DD')
            }
          }
        },
        {
          ...
          customCell: (record: any) => {
            return {
              innerHTML: dayjs(record.serviceTime).format('YYYY-MM-DD')
            }
          }
        },
        {
        ...
        },
        {
          ...
          customCell: (record: any) => {
            return {
              innerHTML: dayjs(record.majorSubmitTime).format('YYYY-MM-DD')
            }
          }
        },
        {
    ...
    const getData = (type = 0) => {
      ...
      if (state.timeR.length > 0) {
        params.serviceTimeStart = dayjs(state.timeR[0]).format('YYYY-MM-DD 00:00:00')
        params.serviceTimeEnd = dayjs(state.timeR[1]).format('YYYY-MM-DD 23:59:59')
      }
