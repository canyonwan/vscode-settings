<script setup lang="ts">
  ...
  function onConfirmTransferout() {
    console.log('%c [ onConfirmTransferout ]-34', 'font-size:13px; background:#0095FF; color:white;', state.causeListTemp)
    state.causeList = state.causeListTemp.map((i) => {
      const res = i.split('&')
      return {
        id: res[0],
        caseType: res[1]
...
<template>
  <n-checkbox-group v-model:value="state.causeListTemp" v-for="i in state.causes" :key="i.caseType" class="py-2">
    ...
    <n-collapse class="pl-4" accordion arrow-placement="right">
      <n-checkbox :value="`${e.id}&${e.caseType}`" v-for="e in i.causeList" :key="e.id">
        <n-collapse-item :title="e.name" :name="e.id">
