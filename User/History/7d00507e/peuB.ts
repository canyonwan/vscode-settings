...
export const schemas = [
...

  {
    ...
    component: 'NInput',
    label: '执法证号',
    showDisplayControl: 'isEnforce',
    componentProps: {},
    rules: [{ required: true, message: '请输入执法证号', type: 'string', trigger: ['blur', 'change'] }]
  },
  {
    field: 'enforceFileList',
    component: 'NUpload',
    label: '执法证照片',
    showDisplayControl: 'isEnforce',
    labelMessage: '注：此项会入卷，请上传清晰完整的执法证复印件',
    componentProps: {
      maxNumber: 1
    },
    rules: [
      {
        ...
        trigger: 'blur',
        validator: (_, value) => {
          if (!value) return new Error('请上传执法证照片')
          return true
        }
      }
    ]
  ...
  },
  {
    ...
    component: 'NInput',
    label: '监督证号',
    showDisplayControl: 'isSuperviser',
    componentProps: {},
    rules: [{ required: true, message: '请输入监督证号', type: 'string', trigger: ['blur', 'change'] }]
  },
  {
    field: 'isMajorLaw',
    component: 'NRadioGroup',
    ...
    rules: [
      {
        ...
        validator: (_rule, value) => {
          if (value === null) {
            return new Error('请选择是否取得法律职业资格')
          }
        }
      }
    ]
