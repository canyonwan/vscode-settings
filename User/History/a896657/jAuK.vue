<script setup lang="ts">
  ...
  function handleUpload(_: IFileItem[], file: any) {
  ...
  function handleSubmit() {
    formRef.value.validate(async (err) => {
      if (!err) {
        await importCause({ file: searchState.fileTemp, deptList: searchState.deptList })
        closeModal()
      }
    })
  }
...
<template>
  <div class="import-cause">
    <n-form :model="searchState" ref="formRef" :rules="rules" :inline="false">
      <n-form-item ref="deptRef" label="部门类型" label-placement="left" path="searchState.deptList">
        <n-space vertical>
      ...
      <n-form-item ref="fileRef" label="上传文件" label-placement="left" path="searchState.fileTemp">
