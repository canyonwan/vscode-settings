<script setup lang="ts">
  ...
  async function calcTiming() {
    const { calcLoading } = await useCalcMoney(calcMoneyState!)
    loading.value = calcLoading.value
  }
  ...
  const debounceFn = useDebounceFn(async (value) => {
    ...
    for (const ele of calcMoneyState!.checkedDiscretionDetail) {
      if (ele.id === state.value!.id) {
        ...
        const { calcLoading } = await useCalcMoney(calcMoneyState!)
        console.log('%c [item calcLoading ]-166', 'font-size:13px; background:#0095FF; color:white;', calcLoading.value)
        loading.value = calcLoading.value
      }
    }
  }, 1000)
