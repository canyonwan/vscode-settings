<script setup lang="ts">
  ...
  function echoCalcMoneyState() {
    console.log('%c [ multiply(item.operatorValue!, 100) ]-170', 'font-size:13px; background:#0095FF; color:white;', multiply(0.07, 100))
    console.log('%c [ add(item.operatorValue!, 100) ]-170', 'font-size:13px; background:#0095FF; color:white;', add(0.1, 0.2))

    // for (const item of calcMoneyState!.checkedDiscretionDetail) {
    //   if (item.id === state.value!.id) {
    //     if (state.value?.unit === 2) {
    //       console.log('%c [ item ]-174', 'font-size:13px; background:#0095FF; color:white;', item)
    //       // state.value!.operatorValue = Number((item.operatorValue! * 100).toFixed())
    //       state.value!.operatorValue = multiply(item.operatorValue!, 100)
    //       return
    //     }
  ...
  <div class="d-menu-item ml-4">
    <div v-if="state?.whetherGroupType === 1">
      <div class="m-4 text-purple-500">
        ...
        {{ `${item.name}-${item.id} (${item.selectType === 1 ? '单选' : '多选'})` }}
      </div>
      ...
      <div class="mb-4 p-2 bg-highlightBgColor rounded">
        <n-space justify="space-between" v-if="state?.parentId !== '0'">
          <n-checkbox v-if="state?.selectType === 2" :checked="state.isChecked === 1" @update:checked="handleUpdateChecked">{{
            `${state?.name} ${state.id} (${state.selectType === 1 ? '单选' : '多选'})`
          }}</n-checkbox>
