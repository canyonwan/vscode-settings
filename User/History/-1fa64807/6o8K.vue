<script setup lang="ts">
  ...
  async function getCauseListForProgram() {
    ...
    const { causeList } = res.filter((item) => item.caseType === 101)[0]
    causeListData.value = causeList
    defaultTabValue.value = causeList[0].id
  }
  ...
  <n-card class="discretion" ref="el">
    ...
    <div v-else>
      <n-tabs type="bar" animated size="large" v-model:value="defaultTabValue" @before-leave="handleTabBeforeLeave">
        <n-tab-pane v-for="item in causeListData" :key="item.id" :name="item.id" display-directive="if">
          <template #tab>
