<script setup lang="ts">
  ...
  import { deleteSubCause, queryCauseManageList, causeBindCauseType, queryBoundCauseTypeForCause } from '@/api/cause-manage/cause-manage'
  ...
  const tableRef = ref()
  const causeId = ref('')
  const boundCauseTypes = ref<string[]>([])
  ...
  const createActions = (record: ICauseSubListItemType) => {
    return [
      ...
      },
      {
        ...
        label: '绑定违法行为类型',
        frontEndIdentification: ['b01_01_02'],
        onClick: onBindCauseType.bind(null, record)
      },
      {
        type: 'error',
        label: '删除',
  ...
  function onAddCause() {
    causeId.value = ''
    setProps({ title: '新增违法行为' })
    currentModalContent.value = defineAsyncComponent(() => import('@/views/cause-library/manage/components/save-cause.vue'))
    openModal()
  }

  function onImportCause() {
    causeId.value = ''
    setProps({ title: '导入违法行为' })
    currentModalContent.value = defineAsyncComponent(() => import('@/views/cause-library/manage/components/import-cause.vue'))
    openModal()
  }

  function onEdit(row: ICauseSubListItemType) {
    causeId.value = row.id!
    setProps({ title: '编辑违法行为' })
    currentModalContent.value = defineAsyncComponent(() => import('@/views/cause-library/manage/components/save-cause.vue'))
    openModal()
  }

  // query bound cause types
  async function getBoundCauseTypes(id: string) {
    const res = await queryBoundCauseTypeForCause(id)
    boundCauseTypes.value = res.map((e) => e.id)
    console.log('%c [ boundCauseTypes.value ]-127', 'font-size:13px; background:#0095FF; color:white;', boundCauseTypes.value)
  }

  function onBindCauseType(row: ICauseSubListItemType) {
    causeId.value = row.id!
    setProps({ title: '绑定违法行为类型' })
    currentModalContent.value = defineAsyncComponent(() => import('@/views/law/components/cause-type.vue'))
    getBoundCauseTypes(row.id!)
    openModal()
  }

  // 选择违法行为类型
  async function checkedKeysHandler(ids: string[]) {
    await causeBindCauseType({
      sourceId: causeId.value,
      targetList: ids
    })
    closeModal()
  }
...
<template>
  <n-card class="manage-home rounded-tl-3xl">
    ...
    <n-grid x-gap="12" :cols="4" :y-gap="40">
      ...
      <n-gi span="2">
        <n-space>
          <n-button class="mb-4" type="primary" @click="onSelectCauseTags">选择违法行为标签</n-button>
    ...
    <basic-table ref="tableRef" :row-key="(row: ICauseSubListItemType) => row.id" :request="loadTableData" :columns="causeTableColumns" :action-column="actionColumn" :scroll-x="1800">
      <template #toolbar>
        <n-button class="mr-4" icon-placement="right" type="success" @click="onImportCause" v-if="buttonShowPermission(['b01_01_01'])" :disabled="buttonDisabledPermission(['b01_01_01'])">
          <template #icon>
    ...
    <basic-modal @register="saveModalRegister" style="width: 70%">
      <component
        ...
        :id="causeId"
        @on-select-ok="saveCauseHandler"
        @on-close-modal="closeModal"
        @submit-handler="saveCauseHandler"
        use-check
        :checked-keys="boundCauseTypes"
        @checkedKeysHandler="checkedKeysHandler"
      />
    </basic-modal>
  </n-card>
</template>
