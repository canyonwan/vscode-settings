...
<script setup lang="ts">
  ...
  async function getShowEnforceCertDetail() {
    ...
    if (aMap.value) await useGetAddress(aMap.value, map.value, [res.longitude, res.latitude])
  }

  function chooseAddressHandler(params: []) {
    console.log('%c [ params ]-87', 'font-size:13px; background:#0095FF; color:white;', params)
  }
...
<template>
  <div class="show-enforce-certificate">
    <n-form :model="form" ref="formRef" label-placement="left" :rules="rules">
      ...
      <e-map @choose-address-handler="chooseAddressHandler" />
