...
<script setup lang="ts">
  ...
  async function getShowEnforceCertDetail() {
    ...
    nextTick(() => {
      console.log('%c [ mapRef.value.aMap ]-86', 'font-size:13px; background:#0095FF; color:white;', mapRef.value.aMap)
    })
  }
  ...
  onMounted(() => {
    if (mapRef) {
      debugger
      useGetAddress(['114.500101', '38.04809'], mapRef.value.aMap, mapRef.value.mapInstance)
    }
  })
