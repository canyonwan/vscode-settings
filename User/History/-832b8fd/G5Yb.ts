...
export const UseButton = defineComponent({
  ...
  setup(props, { slots }) {
    return () => {
      if (slots.default) return h('div', { class: [`bg-e-button-orange-bg`, 'leading-10 px-4  text-center align-middle text-white cursor-pointer  bg-no-repeat bg-full'] }, slots.default())
    }
  }
})
