...
import { saveSubCauseBaseAttr } from '@/api/cause-manage/cause-manage'
...
interface IState {
  baseAttrsForSelect: SelectOption[]
  groupNumber: number // 分组编号
  whetherGroupBackValue: string // 数据是否组返回
  causeBaseAttrId: string | null
  groupContent: string // 分组内容
}
export default defineComponent({
  ...
  setup(props: HandlingWayCommonPropsType, { emit }) {
    ...
    const resetForm = () => {
      state.groupNumber = 0
      state.groupContent = ''
      state.whetherGroupBackValue = '0'
      state.causeBaseAttrId = null
    }
      ...
      formRef.value?.validate(async (errors) => {
        if (!errors) {
          ...
          const params: ICauseAttrItemType = {
            ...
            causeBasicAttrId: state.causeBaseAttrId as string, // 违法行为基本属性id
            dataGroup,
    ...
    return () => (
      <NForm labelPlacement="left" labelWidth="120" model={state} ref={formRef} rules={rules2}>
        <NFormItem label="违法行为基本属性" path="causeBaseAttrId">
          <NSelect options={state.baseAttrsForSelect} placeholder="选择违法行为基本属性" v-model:value={state.causeBaseAttrId}></NSelect>
        </NFormItem>
