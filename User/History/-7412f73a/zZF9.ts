 ...
 * @param params
 */
export function importPersonnel(params: { file: any; orgId: string }): Promise<Blob> {
  return http.request({
    url: `${serverName}user/import`,
    method: RequestEnum.POST,
    data: params,
    responseType: 'blob',
    headers: { 'Content-Type': 'multipart/form-data;boundary=----WebKitFormBoundary' + new Date().getTime() }
  })
}
