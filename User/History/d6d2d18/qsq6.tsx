...
export default defineComponent({
  ...
  setup(props: HandlingWayCommonPropsType, { emit }) {
    ...
    // 选择机关的回调
    const onSelectOrgsHandler = (orgIds: string[]) => {
    ...
    // 选择部门的回调
    const onSelectDeptHandler = (deptIds: string[]) => {
    ...
    const getGroupDetail = async () => {
      ...
      // state.boundIds = typeId.map((item) => item + '')
      state.boundIds = typeId
      console.log('boundIds', state.boundIds)
    }
      ...
      <>
        ...
        <BasicModal onRegister={modalRegister} style={{ with: '70%' }}>
          {h(currentComp.value, {
            ...
            onOnCheck: onSelectOrgsHandler,
            bindKeys: state.boundIds,
            // 部门
            useSelection: true,
            loadTableData: () => getDepartmentTreeData(),
            onSelectedRowHandler: onSelectDeptHandler,
