 ...
 */
export async function handleNodeToolClick({ isToolBox, nodeToolInfo, procedureData, programId }: ShowContentParam) {
  //工具箱
  if (isToolBox) {
    if (nodeToolInfo.type === 1 || nodeToolInfo.type === 2) {
      const initIds = await getModelTimeLineInit(nodeToolInfo, procedureData, programId)
      // 文书跳转
      return {
        ...
        parentId: procedureData.timeLineId, // 父级时间轴ID
      ...
      return {
        ...
        procedureId: programId, // 程序ID
        timeLineId: procedureData.timeLineId, // 时间轴ID
        dataId: procedureData.id, // 当前程序ID
        dataType: procedureData.procedureType, // 程序类型(1:行政检查2:行政处罚3:行政强制4:移交移送5任务)
        targetType: nodeToolInfo.type, // 关联目标类型
        targetId: nodeToolInfo.id // 关联目标id
      }
    } else {
      // 是其他: 比如罚款裁量
      return {
...
export async function getModelTimeLineInit(nodeToolInfo: IToolInfoExpress, procedureData: IProgramItem, programId: string): Promise<IInitTimeLineInfo> {
  // 文书程序id和时间轴id
  return await initTimeLine({
    parentId: procedureData.timeLineId,
