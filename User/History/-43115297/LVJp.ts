 ...
 * @desc 案由已经绑定的违法行为类型
 */
export function queryBoundCauseTypeForCause(causeId: string): Promise<ICauseType[]> {
  return http.request({
 ...
 * @desc 案由已经绑定的违法行为类型
 */
export function queryBoundCauseTypeForDeptType(deptid: string): Promise<ICauseType[]> {
  return http.request({
    url: `${ServerName}causeTypeBindingDept/causeType/${deptid}`,
    method: RequestEnum.GET
  })
}
