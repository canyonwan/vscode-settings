  ...
  function handleSubmit() {
    formRef.value.validate(async (err) => {
      if (!err) {
        ...
        console.log('%c [ res ]-46', 'font-size:13px; background:#0095FF; color:white;', res)
        const res2 = await blobToString(res)
        console.log('%c [ ret ]-49', 'font-size:13px; background:#0095FF; color:white;', res2)
        // blobToString(res).then((resString) => {
        //   console.log('%c [ resString ]-48', 'font-size:13px; background:#0095FF; color:white;', resString)
        //   const ret = JSON.parse(resString)
        //   console.log('%c [ ret ]-49', 'font-size:13px; background:#0095FF; color:white;', ret)
