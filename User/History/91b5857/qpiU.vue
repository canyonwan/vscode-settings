...
<template>
  <div class="py-4 flex items-center">
    <div class="dashed flex-grow" v-if="show"></div>
    <!-- 未开始 -->
    <n-icon v-if="props.itemNode?.nodeStatus === 0" class="text-gray-600" :component="EllipseOutline" />
    <!-- 进行中 -->
    <n-icon v-if="props.itemNode?.nodeStatus === 1">
      <svg class="animate-spin text-yellow-500" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32">
        <defs />
    ...
    <!-- 已完成 -->
    <n-icon color="#7bdfe7" v-if="props.itemNode?.nodeStatus === 2" :component="CheckmarkCircle" />
    <!-- 标题 -->
    <n-ellipsis style="max-width: 240px" class="text-xs px-2"> {{ props.itemNode?.title }}</n-ellipsis>
    <!-- 连接线 -->
    <div class="dashed flex-grow" v-if="idx !== parentChildrenLength! - 1"></div>
  </div>
  <!-- 文件 -->
  <media-preview class="ml-4" v-show="props.itemNode.fileList.length > 0" :values="props.itemNode.fileList" />
</template>
