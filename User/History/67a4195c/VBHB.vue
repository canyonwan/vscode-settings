  ...
  <div class="case-types">
    <n-grid cols="5" responsive="screen" :x-gap="12">
      <n-gi span="2">
        ...
        >
          <template #header>
            <n-button type="primary" class="mb-1 float-right" @click="addTopLevel" v-if="buttonShowPermission(['b04_04_01'])" :disabled="buttonDisabledPermission(['b04_04_01'])">新增顶级 </n-button>
          </template>
      ...
      <n-gi span="3">
        <n-card class="flex-none h-full custom-from-item" size="small" content-style="padding: 0 10px">
          <n-tabs v-model:value="currentTab" @before-leave="handleTabBeforeLeave" justify-content="space-evenly" default-value="baseInfo" type="line" size="large">
            ...
            <n-tab-pane name="causeBindTab" tab="违法行为绑定">
              <cause-base-bind-tab />
            </n-tab-pane>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      provide('resetRowData', resetRowData)
