    ...
    <n-grid cols="5" responsive="screen" :x-gap="12">
      <n-gi span="2">
        ...
        >
          ...
          <case-type-trees ref="caseTypeTreeRef" @addSubhandler="addSubhandler" @on-edit-row="editRowHandler" @on-delete-row="onDeleteRow" />
        </n-card>
      </n-gi>
      <n-gi span="3">
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      // 编辑: id: id
      function addSubhandler(row: ICaseType) {
        state.currentTab = 'baseInfo'
        state.handleTarget = '新增子级'
        // state.editMode = false
        // state.handleTarget = row?.name ? row.name : '顶级'
        provideRowData.value = row
        console.log('新增按钮回调', provideRowData.value)
      }
