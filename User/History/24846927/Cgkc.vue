<script setup lang="ts">
  ...
  function deleteCurrentCause(e: MouseEvent, caseTypeId: number, causeId: string) {
    e.stopPropagation()
    const params = {
    ...
    <n-collapse class="pl-4" default-expanded-names="1" accordion arrow-placement="right">
      <n-collapse-item v-for="e in i.causeList" :key="e.id" :title="e.name" :name="e.id">
        ...
        <template #header-extra>
          <n-button type="error" @click="(e: MouseEvent) => deleteCurrentCause(e, i.caseType, e.id)" text>删除</n-button>
        </template>
      </n-collapse-item>
    </n-collapse>
  </n-card>
