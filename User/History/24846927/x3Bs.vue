<script setup lang="ts">
  ...
  async function deleteCurrentCause(caseTypeId: number, causeId: string) {
    const params = {
    ...
    dialog.error({
      ...
      negativeText: '取消',
      onPositiveClick: () => {
        deleteApproveData(info.id).then(() => {
          reloadTable()
        })
      },
      onNegativeClick: () => {}
    ...
    <n-collapse class="pl-4" default-expanded-names="1" accordion arrow-placement="right">
      <n-collapse-item v-for="e in i.causeList" :key="e.id" :title="e.name" :name="e.id">
        ...
        <template #header-extra>
          <n-button type="error" @click="deleteCurrentCause(i.caseType, e.id)" text>删除</n-button>
        </template>
      </n-collapse-item>
    </n-collapse>
  </n-card>
