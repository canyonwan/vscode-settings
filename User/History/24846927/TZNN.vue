<script setup lang="ts">
  ...
  function deleteCurrentCause(caseTypeId: number, causeId: string, evt: MouseEvent) {
    evt.stopPropagation()
    const params = {
    ...
    <n-collapse class="pl-4" default-expanded-names="1" accordion arrow-placement="right">
      <n-collapse-item v-for="e in i.causeList" :key="e.id" :title="e.name" :name="e.id">
        ...
        <template #header-extra>
          <n-button type="error" @click="(evt: MouseEvent) => deleteCurrentCause(e.caseTypeId, e.id, evt)" text>删除</n-button>
        </template>
      </n-collapse-item>
    </n-collapse>
  </n-card>
