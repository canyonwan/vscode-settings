<template>
  <n-modal class="bg-modal-bg bg-no-repeat bg-center bg-fixed" id="basic-modal" v-bind="getBindValue" v-model:show="isModal" @close="onCloseModal">
    <template #header>
      <div class="w-full cursor-move" id="basic-modal-bar">{{ getBindValue.title }}</div>
    </template>
    <template #default>
      <n-scrollbar v-if="getBindValue.showActionButtonGroup" :style="{ maxHeight: 'calc(100vh - ' + 160 + 'px)', paddingRight: '20px' }">
...
<script lang="ts" setup>
  ...
  import type { ModalProps, ModalMethods } from './type'
