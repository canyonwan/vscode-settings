<script setup lang="ts">
  ...
  function onTabChange(item: IProgramItem) {
    // currentTimelineId.value !== item.timeLineId && emit('dispatchRelatedProgram', item.timeLineId, item.procedureType, item.id)
    currentTimelineId.value !== item.timeLineId && emit('dispatchRelatedProgram', item)
  ...
  watch(
    ...
    (newValue) => {
      ...
      console.log('%c [ data ]-31', 'font-size:13px; background:#0095FF; color:white;', props.data)
    },
    {
      immediate: true
    }
...
<template>
  <div class="related-program-bar p-1 h-full rounded-lg flex flex-col justify-center">
    <div
      class="item text-white font-medium px-3 text-sm cursor-pointer rounded-lg h-full flex flex-col justify-center"
