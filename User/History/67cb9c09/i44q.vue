...
<template>
  <div class="related-program-bar h-full rounded-lg flex flex-col justify-center">
    <div
      class="item text-white font-medium text-sm cursor-pointer rounded-lg h-full"
      :class="{ 'item-selected': currentTimelineId === item.timeLineId }"
      @click="onTabChange(item)"
      v-for="item in props.data"
      :key="item.id"
    >
      <!-- {{ programTypeMap[item.procedureType] }} -->
      <div v-for="text in programTypeMap[item.procedureType]" :key="text" class="flex flex-col justify-center">
        {{ text }}
      </div>
    </div>
  </div>
