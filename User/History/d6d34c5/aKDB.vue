<template>
  <div>
    ...
    <basic-modal @register="modalRegister" style="width: 70%">
      <component :is="currentComp" @closeModal="() => closeModal()" @on-check="onOrgSelectHandler" :bindKeys="state.boundIds" :use-selection="true" @loadTableData="() => getDepartmentTreeData()" />
      <!-- {h(currentComp.value, {
            // 机关
...
<script setup lang="ts">
  ...
  import { HandlingWayCommonPropsType } from '../types'

  const emits = defineEmits(['cancelEdit', 'submitEdit', 'okSaveGroup'])
  const props = defineProps<HandlingWayCommonPropsType>()
  // 分组类型 1通用2部门3机关
