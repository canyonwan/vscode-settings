<template>
  <div>
    ...
    <basic-modal @register="modalRegister">
      <add-modal @on-submit="onOkModal" :row-id="rowId" />
    </basic-modal>
  </div>
</template>
...
<script lang="ts">
  ...
  import { changeDictionaryStatus, queryDictionariesForList, saveDictionaries } from '@/api/setting/dictionaries'
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const rowId = ref<IDictionaryType>()
      const tableRef = ref()
      ...
      async function handleSave(record) {
        ...
        fetchStatusChanged(record.id, record.status)
        let obj = {
          id: record.id,
          type: record.type,
          remarks: record.remarks,
          displayValue: record.displayValue,
          hiddenValue: record.hiddenValue
        }
        await saveDictionaries(obj)
      }
      ...
      // 状态修改
      const fetchStatusChanged = (id: string, status: boolean) => {
        changeDictionaryStatus({ id: id, status: status ? 0 : 1 }).then(() => {
          reloadTable()
        })
      }
