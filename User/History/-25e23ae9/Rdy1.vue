<script setup lang="ts">
  import type { ICustomDiscretionFormalParameter, IDiscretionMenuItem, IDiscretionOptions, IProgramItem } from '@/api/program/types'
  ...
  // let discretionBaseState = reactive<IDiscretionBaseState>({
  //   discretionBaseId: '',
  //   baseValue: 0 // 基准值
  // })

  // const bus = useEventBus(discretionBaseStateEventKey)
  // bus.on((data: IDiscretionBaseState) => {
  //   discretionBaseState.discretionBaseId = data.discretionBaseId
  //   discretionBaseState.baseValue = data.baseValue
  ...
  const customDiscretionList: ICustomDiscretionFormalParameter[] = reactive([])

  function addCustomDiscretion() {
    customDiscretionList.push({
      proceduresCauseId: props.programCauseId,
      name: '',
      remarks: '',
      operatorType: 1, // 运算方式 1加 2减 3乘4除
      operatorValue: 0, // 自定义的值
      unit: 1
    })
  }
  findOption(discretionDetailList)

  function handleSubmit(values: ICustomDiscretionFormalParameter) {
...
<template>
  <div class="discretion-menu">
    <div v-for="item in discretionDetailList" :key="item.id">
      <!-- 是组区分 -->
      <div v-if="item.whetherGroupType === 1">
    ...
    <basic-modal @register="modalRegister" style="width: 60%">
      <custom-discretion :programCauseId="props.programCauseId" @handleSubmit="handleSubmit" />
    </basic-modal>
  </div>
</template>
