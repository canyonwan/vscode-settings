...
<template>
  <n-space>
    <div
      ...
      :key="item.fileId"
      class="rounded-md hover:ring-4 transition duration-500 ease-in-out border p-1 info mr-1 mb-1 cursor-pointer"
      @click="onPreview(item.fileUrl, item.fileType)"
    >
      <img class="relative w-20 h-20" v-if="item.thumbnailFileUrl" :src="item.thumbnailFileUrl || item.fileUrl" />
      <img class="relative w-20 h-20" v-else-if="!item.thumbnailFileUrl && item.fileType === FileTypeEnum.Picture" :src="item.fileUrl" />
      ...
      <div class="bg-blue-gray-500 absolute">mark</div>
    </div>
