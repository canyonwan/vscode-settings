...
<template>
  <n-space>
    <!-- v-if="item.fileType === FileTypeEnum.Picture || item.fileType === FileTypeEnum.Pdf || item.fileType === FileTypeEnum.Word || item.fileType === FileTypeEnum.Video" -->
    <div v-for="item in fileData" :key="item.fileId" class="rounded-md border p-1 info mr-1 mb-1 cursor-pointer hover:bg-gray-900" @click="onPreview(item.fileUrl, item.fileType)">
      <img v-if="item.thumbnailFileUrl" :src="item.thumbnailFileUrl || item.fileUrl" class="w-20 h-20" />
      <img v-else-if="!item.thumbnailFileUrl && item.fileType === FileTypeEnum.Picture" :src="item.fileUrl" class="w-20 h-20" />
