export interface IProgramItem {
  ...
  unlawfulAct: string
  relevanceList?: IProgramItem[]
  hostOrgId?: string
  hostOrgName?: string
  hostOrganId?: string
  hostOrganName?: string
...
export interface ICalcDiscretionSumFormalParameter {
  proceduresCauseId: string // 程序违法行为ID
...
export interface ICauseList {
  ...
  caseType: string
  caseTypeName: string
  causeSubId?: string // 违法行为id(原始id)
  identifier?: string // 违法行为编号
  transferOut?: number // 0:未转出过1:已经转出
  buttonList?: IButtonList[] // 后端可能已改为tipList
  tipList: ITipList[]
}
...
// 查询没收裁量下的证据项列表
export interface IEvidenceDataForConfiscateDiscretion {
  evidenceCustomList: IEvidenceItemForConfiscateDiscretion[]
  evidenceList: IEvidenceItemForConfiscateDiscretion[]
}

export interface IEvidenceItemForConfiscateDiscretion {
  id: string
  causeId: string
