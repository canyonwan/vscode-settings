  ...
  <n-form v-bind="getBindValue" :model="formModel" ref="formElRef">
    <n-grid v-bind="getGrid">
      <n-gi v-bind="schema.giProps" v-for="schema in getSchema" :key="schema.field">
        ...
        ></slot>
        <n-form-item :label="schema.label" :label-width="schema.labelWidth" :path="schema.field" v-else-if="schema['showDisplayControl'] && formModel[schema['showDisplayControl']] === '1'">
          {{ formModel }}
          {{ formModel[schema['showDisplayControl']] }}
          <!--标签名右侧温馨提示-->
