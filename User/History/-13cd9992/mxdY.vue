  ...
  <n-form v-bind="getBindValue" :model="formModel" ref="formElRef">
    <n-grid v-bind="getGrid">
      <n-gi v-bind="schema.giProps" v-for="schema in getSchema" :key="schema.field">
        ...
        <n-form-item :label="schema.label" :label-width="schema.labelWidth" :path="schema.field" v-else-if="formModel[schema['showDisplayControl']]">
          {{ `这个: ${formModel[schema['showDisplayControl']]}` }}
