    ...
    <n-space vertical>
      <n-radio-group v-model:value="discretionBaseState.discretionBaseId" name="radiogroup" @update:value="onRadioGroupChanged">
        <n-space vertical v-for="item in injectDiscretionOptions.discretionList" :key="item.id" class="my-4">
          ...
          <div class="bg-highlightBgColor p-2 rounded" v-for="ele in 3" :key="ele">
            <!-- <div class="py-3" v-for="item in ele.evidenceDetailedList" :key="item.id">
              <div v-for="i in item.fileInfoOutputs" :key="i.id"> <img :src="i.fileInfoOutput.fileUrl" /> </div>
            </div> -->
            <n-space>
              <n-image-group>
                <n-space>
                  <!-- <n-image v-for="ele in evidences" :key="ele.id" width="100" :src="ele.fileInfoOutput.fileUrl" /> -->
                  <n-image v-for="ele in 3" :key="ele" width="100" src="https://07akioni.oss-cn-beijing.aliyuncs.com/07akioni.jpeg" />
                </n-space>
              </n-image-group>
