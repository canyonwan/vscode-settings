<script setup lang="ts">
  ...
  const [evidenceModalRegister, { openModal }] = useModal({ title: '采集证据' })
  function onNextStep() {
  ...
  const evidenceInfo = reactive<IEvidenceConnect>({
    ...
    typeIdName: '',
    toolType: '',
    // procedureId: parseInt(injectProgramId.value),
    procedureId: discretionBaseState.baseValue,
    relationType: '3', // 2要素 3: 类型
    timeLineId: programDetail!.timeLineId,
    causeId: props.programCauseId,
    causeRuleId: discretionBaseState.discretionBaseId, // 选中的裁量规则id
    causeDiscretionId: ''
  })

  function onRadioGroupChanged(value: string) {
    discretionBaseState.discretionBaseId = value
    console.log('onRadioGroupChanged', value)
    console.log('cause rule id', evidenceInfo)
    console.log('discretionBaseState', discretionBaseState)
  }
</script>
...
<template>
  <div class="discretion-type bg-contentBgColor">
        ...
        <n-space vertical v-for="item in injectDiscretionOptions.discretionList" :key="item.id" class="my-4">
          <n-space justify="space-between">
            ...
            <n-space class="pl-1 text-gray-400 text-xs leading-5">
              ...
              <n-button strong secondary size="small" type="primary" @click="() => openModal()">上传材料</n-button>
            </n-space>
          </n-space>
          <!-- 证据列表 -->
          <div class="bg-highlightBgColor p-2 rounded" v-for="ele in item.discretionEvidenceList" :key="ele.id">
            <n-space justify="space-between">
              <div>{{ ele.name }}</div>
            </n-space>
          </div>
    ...
    <basic-modal @register="evidenceModalRegister" style="width: 70%">
      <evidence-select :isSelect="true" :case-info="evidenceInfo" />
    </basic-modal>
  </div>
</template>
