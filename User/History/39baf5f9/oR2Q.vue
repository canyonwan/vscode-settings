<template>
  <div class="sub-cause">
    <n-alert type="info" closable class="mb-10"> {{ data?.id ? `正在编辑【${editingCauseName} 】` : `正在为【${editingCauseName}】新增子违法行为` }} </n-alert>
    <n-form label-placement="left" label-width="100px" :style="{ width: '80%' }">
      <!-- 违法行为编号-->
      <n-form-item label="违法行为编号">
        <n-input v-model:value="data.identifier" placeholder="请输入违法行为编号" />
      </n-form-item>
      <!-- 违法行为名称 通用-->
      <n-form-item label="违法行为名称">
        <n-input type="textarea" v-model:value="data.name" placeholder="请输入违法行为名称" />
      </n-form-item>
      <!-- 检查对象类型 -->
      ...
      <!--主违法行为详情-->
      <n-form-item label="违法行为法">
        <n-space vertical>
          <n-button type="primary" @click="onSelectCauseLaw">违法行为法</n-button>
          <n-space>
...
<script lang="ts">
  ...
  import { causeEventKey } from '@/views/cause-library/manage/outdated/fooKey'
  ...
  import { queryDeptList, queryLawDetailList } from '@/api/law/cause'
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const state = reactive({
        ...
        data: {} as ICauseSubListItemType, // 违法行为详情数据
        illegalSubjectOptions: [] as string[],
        editingCauseName: '' as string | undefined,
        causeTagTreeData: [] as ILawTagType[], // 违法行为标签树Data
        boundCauseLawKeys: [] as string[], // 选择的违法行为法keys
        boundCauseLaws: [] as Partial<ILawItemType>[], // 选择的违法行为法keys
      ...
      async function onSelectDepartmentHandler(keys: string[]) {
        state.boundDepartmentTypes = keys
        const deptNames = await queryDeptList(keys)
        state.departmentTypes = deptNames
        closeModal()
      }
      ...
      watchEffect(() => {
        ...
        // 检查对象类型
        state.selectedCheckingTargetType = props.causeDetail?.penaltyObjectType ? props.causeDetail!.penaltyObjectType!.split(',') : []
        // 违法行为法
        ...
        // 机关层级
        state.orgPropertiesOptions = props.causeDetail?.orgProperties ? props.causeDetail!.orgProperties!.split(',') : []
        // 适用提示
