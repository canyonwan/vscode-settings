    ...
    () => {
      if (mapInstance.value) {
        mapInstance.value.on('click', async (e: any) => {
          ...
          const res = await useGetAddress(aMap.value, mapInstance.value, [lnglat.lng, lnglat.lat])
          console.log('%c [ res ]-16', 'font-size:13px; background:#0095FF; color:white;', res)
          emit('chooseAddressHandler', [res, lnglat])
