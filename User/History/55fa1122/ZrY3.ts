...
export function useSearch(reloadTable: Function) {
  ...
  async function selectCauseTagsHandler(values: string[], rows: ILawTagType[]) {
    console.log('%c [ rows ]-83', 'font-size:13px; background:#0095FF; color:white;', rows)
  ...
  async function queryCauseTags() {
    ...
    console.log('%c [ causeTagDatasources.value ]-103', 'font-size:13px; background:#0095FF; color:white;', causeTagDatasources.value)
  }
