...
export function useSearch(reloadTable: Function) {
  ...
  // 删除单个案由法
  function onDeleteSingleLaw(id: string) {
    // displayState.selectedLaws.splice(displayState.selectedLaws.indexOf(item), 1)
    searchParams.lawIdList?.splice(searchParams.lawIdList.indexOf(id), 1)
  }
  ...
  // 删除单个案由标签
  function onDeleteSingleCauseTag(id: string) {
    displayState.selectedLaws.splice
    searchParams.causeTagIdList!.splice(displayState.selectedLaws.indexOf(id), 1)
  }
