import { queryCauseTagsWithIds } from '@/api/cause-manage/outdated/cause-manage'
...
export function useSearch(reloadTable: Function) {
  ...
  function onSelectLaw() {
    ...
    console.log('%c [ si ]-47', 'font-size:13px; background:#0095FF; color:white;', searchParams.lawIdList)
  }
  ...
  async function selectLawHandler(keys: string[], rows: ILawItemType[]) {
    ...
    console.log('%c [  displayState.selectedLaws ]-52', 'font-size:13px; background:#0095FF; color:white;', displayState.selectedLaws)
  ...
  // 选择案由标签回调
  async function selectCauseTagsHandleer(values: string[]) {
