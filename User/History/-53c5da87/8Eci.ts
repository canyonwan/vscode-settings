...
import { FormItemRule, NSelect } from 'naive-ui'
    ...
    rules: [
      {
        ...
        validator: (rule: FormItemRule, value: string) => {
          console.log('%c [ rule ]-89', 'font-size:13px; background:#0095FF; color:white;', rule)
          console.log('%c [ value ]-89', 'font-size:13px; background:#0095FF; color:white;', value)
          if (!value) return new Error('请选择案由法')
          return true
        }
      }
    ]
    ...
    rules: [
      {
        ...
        trigger: 'blur',
        validator: (_, value) => {
          if (!value) return new Error('请选择部门类型')
          return true
        }
      }
    ]
