...
import { programRecordType } from './map-options'
...
export const punishColumns = [
  ...
  },
  {
    ...
    width: 200,
    render: (rowData: any) => {
      if ([null, ''].includes(rowData?.serviceDate)) {
        console.log('%c [ if ]-105', 'font-size:13px; background:#0095FF; color:white;', rowData?.serviceDate)
        // return ''
      } else {
        console.log('%c [ else ]-105', 'font-size:13px; background:#0095FF; color:white;', rowData?.serviceDate)
        // return format(rowData.serviceDate, 'yyyy-MM-dd')
      }
      // return [null, ''].includes(rowData?.serviceDate) ? '' : format(rowData.serviceDate, 'yyyy-MM-dd')
    }
  },
  {
    title: '结案日期',
