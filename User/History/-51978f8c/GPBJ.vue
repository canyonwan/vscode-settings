<script setup lang="ts">
  ...
  const [modalRegister, { openModal, closeModal }] = useModal({
    displayDirective: 'if',
    title: `${bannerId.value ? '编辑' : '新增'}轮播图`
  })
  ...
  // 查询
  async function onSearch(values: IBannerItem) {
...
<template>
  <div class="base-home">
    ...
    <basic-table :row-key="(row:any ) => row.id" :request="loadTableData" ref="tableRef" :columns="bannerColumns" :action-column="actionColumn" scroll-x="1090">
      <template #toolbar>
        <n-button type="primary" @click="onAdd"> 新增轮播图 <n-icon :component="Add" size="22" /></n-button>
      </template>
    </basic-table>
    <basic-modal @register="modalRegister" style="width: 70%">
      <save-detail :id="bannerId" @handle-save="handleSave" />
    </basic-modal>
  </div>
</template>
