  ...
  <div class="sub-cause">
    ...
    <n-form label-placement="left" label-width="100px" :style="{ width: '80%' }">
      ...
      <n-form-item label="违法主体标签">
        <!-- <n-input v-model:value="data.illegalSubject" placeholder="请输入违法主体标签" /> -->
        <!-- <n-select v-model:value="illegalSubjectOptions" filterable multiple tag placeholder="输入，按回车确认" :show-arrow="false" :show="false" /> -->
...
<script lang="ts">
  ...
  import { queryAssignLaw } from '@/api/law/cause'
    ...
    setup(props) {
      ...
      async function submitCauseLawHandler(keys: string[]) {
        ...
        for (const key of keys) {
          let res = await queryAssignLaw(key)
          arr.push({
            id: res.id,
            lawName: res.lawName
          })
      ...
      async function onSelectDepartmentHandler(keys: string[]) {
        ...
        console.log('boundDepartmentTypes', state.boundDepartmentTypes)
        closeModal()
      }
