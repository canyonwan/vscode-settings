    ...
    setup(props, { emit }) {
      ...
      watchEffect(() => {
        if (props.useSelection) {
          if (tableColumnsRef.value[0].type !== 'selection') {
      ...
      // 选择列
      function handleCheck(keys: string[], rows: ILawItemType[]) {
        state.selectedRowKeys = keys
        state.selectedRows = rows
        console.log('%c [ state.selectedRows ]-246', 'font-size:13px; background:#0095FF; color:white;', state.selectedRows)
      }

      // 提交选中的列,回调给调用方
      function submitSelectionHandler() {
        emit('submitSelectionHandler', state.selectedRowKeys, state.selectedRows)
      }
