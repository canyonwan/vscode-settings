<script setup lang="ts">
  ...
  const emits = defineEmits(['emitWidth'])

  const dialog = useDialog()
  const [modalRegister, { openModal, closeModal, setProps }] = useModal({ title: '新增同级节点' })
  const [singleDeleteModalRegister, { openModal: openSingleDeleteListModal }] = useModal({ title: '删除节点' })
  ...
  const timelineNode = ref<ISaveTimelineNodeParams>()
  ...
  function saveNode(nodeType: string) {
    ...
    timelineNode.value = {
      nodeType
      // timeLineId: props.itemNode.tim
    }
    // console.log('i', props.itemNode.title)
    openModal()
  }
  ...
  function handleConfirm() {
    dialog.warning({
      ...
      onNegativeClick: () => {
        openSingleDeleteListModal()
        // let arr = ['您是要删除该节点内全部内容还是单个内容？', '哈哈？']
        // handleSingleConfirm(arr)
      }
    })
  }
  // 删除单个
  function handleSingleConfirm(arr: any[]) {
    dialog.warning({
      title: '温馨提示',
      content: () => {
        return h(
...
<template>
  <div ref="itemEl">
    <n-space align="center">
      ...
      <n-space vertical>
        ...
        <div class="timeline-item bg-darkPrimary p-6 rounded-lg mr-4">
          ...
          <!-- 文件 -->
          <n-space>
            <div v-for="file in props.itemNode?.fileList" :key="file.fileId" class="ml-5 w-16 py-2">
              <!-- img -->
    ...
    <basic-modal @register="modalRegister" style="width: 50%">
      <save-timeline-node :timelineNode="timelineNode" @handleSaveNode="handleSaveNode" />
    </basic-modal>
