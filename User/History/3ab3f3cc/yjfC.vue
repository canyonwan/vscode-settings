<script setup lang="ts">
  ...
  const [singleDeleteModalRegister, { openModal: openSingleDeleteListModal, closeModal: closeSingleDeleteModal }] = useModal({ title: '删除节点', displayDirective: 'if' })
  ...
  function handleSingleDeleteSaveNode() {
    emits('handleRefreshTimeTree')
    if (props.itemNode.childrenList.length === 0) {
    }
  }
...
<template>
  <div ref="itemEl">
    ...
    <basic-modal @register="singleDeleteModalRegister" style="width: 50%">
      <single-delete-list :children="props.itemNode.childrenList" @handleSaveNode="handleSingleDeleteSaveNode" />
    </basic-modal>
  </div>
</template>
