...
<template>
  <div class="node-leaf">
    ...
    <div v-if="props.itemNode?.childrenList">
      ...
      <!-- 连接线 -->
      <!-- <div class="dashed" v-if="idx !== parentChildrenLength! - 1"></div> -->
      <n-divider vertical dashed />
      <!--  -->
      <div class="flex">
        <n-divider dashed class="w-6" />
        <node-leaf class="ml-4" v-for="(ele, idx) in props.itemNode.childrenList" :key="ele.id" :item-node="ele" :idx="idx" :parentChildrenLength="props.itemNode.childrenList.length" />
      </div>
    </div>
    <!-- 只有一层节点 -->
