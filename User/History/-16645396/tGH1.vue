...
<template>
  <div class="node-leaf">
    ...
    <!-- 最后一层节点 -->
    <leaf no-child v-if="!props.itemNode?.childrenList" :item-node="props.itemNode" :idx="props.idx" :parentChildrenLength="props.parentChildrenLength" />
    <media-preview v-if="!props.itemNode?.childrenList && props.itemNode.fileList.length > 0" class="ml-7 mt-4" :values="props.itemNode.fileList" />
  </div>
</template>
