...
<script lang="ts">
import { FileType, StateType, UserType } from '@/@types/index'
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const state = reactive({
      ...
      formState: {
        registrationTimeList: [] as any as TimeRangeValueType, // 报名时间
        trainAndExamTimeList: [] as any as TimeRangeValueType, // 培训及考试时间
        trainName: '', //培训名称
        signUpStartTime: '', //报名开始时间
        signUpEndTime: '', //报名结束时间
        trainStartTime: '', //培训开始时间
        trainEndTime: '', //培训结束时间
        fileList: [] as FileType[], //文件路径
        orgIds: [], //机关IDs
        remarks: '' //备注
    ...
    const onSubmit = () => {
      ...
      if (state.formState.registrationTimeList.length > 0 || state.formState.trainAndExamTimeList.length > 0) {
        params.signUpStartTime = parseTime(state.formState.registrationTimeList[0])
        params.signUpEndTime = parseTime(state.formState.registrationTimeList[1])
        params.trainStartTime = parseTime(state.formState.trainAndExamTimeList[0])
        params.trainEndTime = parseTime(state.formState.trainAndExamTimeList[1])
      }
