    ...
    <a-spin :spinning="loading">
      <a-form ref="formRef" :model="formState" :rules="rules" :labelCol="{ flex: '120px' }" :wrapperCol="{ flex: '1' }">
        ...
        <a-form-item label="报名时间" name="registrationTimeList" required>
          <a-range-picker v-model:value="formState.registrationTimeList" style="width: 220px" allowClear />
        </a-form-item>
        <a-form-item label="培训及考试时间" name="trainAndExamTimeList" required>
          <a-range-picker v-model:value="formState.trainAndExamTimeList" style="width: 220px" allowClear />
        </a-form-item>
...
import { fetchInitiateTraining, InitiateTrainingType, FileItem } from '@/api/subject/law-test'
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const state = reactive({
      ...
      formState: {
        registrationTimeList: [], // 报名时间
        trainAndExamTimeList: [], // 培训及考试时间
        trainName: '', //培训名称
