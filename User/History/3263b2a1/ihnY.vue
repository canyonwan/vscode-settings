...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const onSubmit = () => {
      console.log('%c [ registrationTimeList ]-105', 'font-size:13px; background:#0095FF; color:white;', parseTime(state.formState.registrationTimeList[0]))
      ...
      if (state.formState.registrationTimeList.length > 0 || state.formState.trainAndExamTimeList.length > 0) {
        ...
        params.signUpStartTime = parseTime(state.formState.registrationTimeList[1])
        params.signUpStartTime = parseTime(state.formState.trainAndExamTimeList[0])
        params.signUpStartTime = parseTime(state.formState.trainAndExamTimeList[1])
      }
