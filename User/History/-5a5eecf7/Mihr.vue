<script setup lang="ts">
  ...
  const rowData = ref<IAppPackageItem>()
...
<template>
  <div class="">
    <basic-table :request="loadTableData" ref="tableRef" :columns="tableColumns" :action-column="actionColumn" :scroll-x="1600" :row-key="(row: IAppPackageItem) => row.fileId ">
      <template #tableTitle>
        <n-select :options="appTypeOptions" v-model:value="searchParams.appType" :consistent-menu-width="false" />
        <n-button type="success" @click="onAdd">查询<n-icon :component="Add" /></n-button>
      </template>
      <template #toolbar>
