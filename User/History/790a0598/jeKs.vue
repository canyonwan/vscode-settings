    ...
    <div v-for="group in toolGroups" :key="group.id!">
      ...
      <div v-if="group.currentPermissionsIsRelease && judgeExpress(group, 1)" :disabled="judgeExpress(group, 2)">
        <div v-if="group && group.tools && group.tools.length" class="mb-10">
          ...
          <div style="color: #8dc5ff" class="mt-2 mb-5 block text-sm"> {{ group.name }}</div>
          <n-space v-for="tool in group.tools" :key="tool!.id!" inline :class="['mr-4', !isDisabled && !group.disabled && judgeExpress(tool, 2) ? 'cursor-pointer' : 'cursor-not-allowed']">
            <div v-if="tool && tool.currentPermissionsIsRelease && judgeExpress(tool, 1)">
              <!-- PC端不能做的提示 -->
              <n-popover placement="bottom" trigger="click" :show-arrow="false" :style="{ maxWidth: '240px', backgroundColor: '#ffffff' }" v-if="tool.type && [6].includes(tool.type)">
                <template #trigger>
                  <!-- @click="onClick(tool)" -->
                  ...
                  <div
                    :class="[
                      { 'cursor-not-allowed ': isDisabled || group.disabled || !judgeExpress(tool, 2) },
                      'flex items-center  py-1 px-3 rounded-lg text-gray-500  border border-opacity-25 border-gray-500'
                    ]"
                  >
                    <n-icon size="20" color="rgba(107, 114, 128, 0.5)" v-if="tool.webFileName" class="mr-2">
              ...
              <div
                ...
                @click="onClick(tool)"
                :class="[
                  'mb-3 items-center flex py-1 px-3 rounded-lg',
                  !isDisabled && !group.disabled && judgeExpress(tool, 2) ? 'action-button pointer-events-auto ' : 'text-gray-500  border border-opacity-25 border-gray-500 pointer-events-none'
                ]"
              >
                <!-- <n-icon v-if="tool.webIconFileInfo" id="icon" class="mr-2" color="#0e7a0d">
                </n-icon> -->
                <n-icon size="20" :color="!isDisabled && !group.disabled && judgeExpress(tool, 2) ? '#71bfeb' : 'text-gray-500'" v-if="tool.webFileName" class="mr-2">
                  <e-svg-icon :name="tool.webFileName" />
                </n-icon>
                {{ tool.name }}
              </div>
