          ...
          <n-space v-for="tool in group.tools" :key="tool!.id!" inline :class="['mr-4', !isDisabled && !group.disabled && judgeExpress(tool, 2) ? 'cursor-pointer' : 'cursor-not-allowed']">
            <div v-if="tool && tool.currentPermissionsIsRelease && judgeExpress(tool, 1)">
              ...
              <div
                ...
                @click="onClick(tool)"
                :class="[
                  'mb-2 items-center flex',
                  !isDisabled && !group.disabled && judgeExpress(tool, 2)
                    ? 'action-button pointer-events-auto py-1 px-3'
                    : 'text-textColorDisable py-1 px-3 border rounded-lg border-gray-500 pointer-events-none'
                ]"
              >
