          ...
          <n-space v-for="tool in group.tools" :key="tool!.id!" inline :class="['mr-4', !isDisabled && !group.disabled && judgeExpress(tool, 2) ? 'cursor-pointer' : 'cursor-not-allowed']">
            <div v-if="tool && tool.currentPermissionsIsRelease && judgeExpress(tool, 1)">
              ...
              <div
                v-else
                @click="onClick(tool)"
                :class="['mb-2 items-center flex', !isDisabled && !group.disabled && judgeExpress(tool, 2) ? 'action-button pointer-events-auto' : 'text-textColorDisable pointer-events-none']"
              >
                <!-- <n-icon v-if="tool.webIconFileInfo" id="icon" class="mr-2" color="#0e7a0d">
                </n-icon> -->
                <!-- <n-icon size="20" :color="!isDisabled && !group.disabled && judgeExpress(tool, 2) ? '#71bfeb' : '#8E9DA6'" v-if="tool.webFileName" class="mr-2"> -->
                <n-icon size="20" color="#71bfeb" v-if="tool.webFileName" class="mr-2">
                  <e-svg-icon :name="tool.webFileName" />
                </n-icon>
                {{ tool.name }}
              </div>
