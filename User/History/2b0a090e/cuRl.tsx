import { bindCauseTagsForPrimaryCause, queryAllCauseTags, queryCauseTagsFromPrimaryCause, saveCause } from '@/api/cause-manage/cause-manage'
...
export default defineComponent({
  ...
  setup(props: HandlingWayCommonPropsType) {
    ...
    const state = reactive({
      data: {} as IPrimaryCauseItemType, // 违法行为详情数据
      boundCauseTags: [] as ILawTagType[], // 已绑定的违法行为标签
      boundCauseTagKeys: [] as string[], // 已绑定的违法行为标签,用于回显Tree
      ...
      return (
        <div>
          <NForm label-placement="left" labelWidth="auto">
            {/* cause name */}
            <NFormItem label="违法行为名称">
              <NInput type="textarea" v-model:value={state.data.name} placeholder="请输入违法行为名称"></NInput>
            </NFormItem>
            <NFormItem label="">
            ...
            {/* cause tag  */}
            <NFormItem label="违法行为标签" v-show={state.data?.id}>
              <NSpace vertical>
