...
import { NButton, NForm, NFormItem, NInput, NInputNumber, NSelect, SelectOption } from 'naive-ui'
  ...
  setup(props: HandlingWayCommonPropsType, { emit }) {
    ...
    watchEffect(() => {
      if (props.baseAttrItem.id) {
        ...
        state.groupName = groupDifferentiation
    ...
    const onSubmit = async () => {
      const { groupName: groupDifferentiation, whetherGroupBackValue: dataGroup, groupContent: content } = state
    ...
    return () => (
      <NForm labelPlacement="left" labelWidth="120">
        <NFormItem label="选择案由基本属性">
          <NSelect options={state.baseAttrsForSelect} placeholder="选择案由基本属性" value={state.causeBaseAttrId}></NSelect>
        </NFormItem>
        <NFormItem label="是否组返回">
          <NSelect options={dataWhetherGroup} v-model:value={state.whetherGroupBackValue}></NSelect>
        </NFormItem>
        <NFormItem label="分组名称">
          <NInputNumber placeholder="请输入分组名称" v-model:value={state.groupName} clearable></NInputNumber>
        </NFormItem>
        <NFormItem label="内容">
