<script setup lang="ts">
  ...
  function handleDiscretionBaseState(state: IDiscretionBaseState) {
    // discretionBaseState = state
    console.log('%c [d-tab-panel discretionBaseState ]-52', 'font-size:13px; background:pink; color:#bf2c9f;', discretionBaseState)
  ...
  <div class="discretion-tab-panel">
    ...
    <div class="bg-contentBgColor rounded-sm p-5">
      ...
      <div class="text-2xl text-blue-500">
        ...
        <n-number-animation class="text text-gray-500" show-separator :to="currentDiscretionResult?.discretionMoney" :precision="2" /><span class="text-sm">&nbsp;元</span></div
      >
      <keep-alive>
        <component
