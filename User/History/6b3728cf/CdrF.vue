<template>
  <div class="flex w-full base-types">
    ...
    <basic-modal @register="causeTypeModalRegister" style="width: 40%">
      <CauseType use-check @checkedKeysHandler="checkedKeysHandler" :checked-keys="state.labelSelectKeys" />
    </basic-modal>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  import { deptBindCauseType, queryBoundCauseTypeForDeptType } from '@/api/cause-manage/cause-manage'
  ...
  // 选择绑定标签
  function selectBindLabel(record: IBaseType, type) {
  ...
  // 选择违法行为类型
  async function bindCauseType(record: IBaseType) {
    state.departmentTypeId = record.id
    await getBoundCauseTypes(record.id)
    openCauseTypeModal()
  }

  // 选择违法行为类型
  async function getBoundCauseTypes(id: string) {
    const res = await queryBoundCauseTypeForDeptType(id)
    state.labelSelectKeys = res.map((e) => e.id)
    console.log('%c [ state.labelSelectKeys ]-125', 'font-size:13px; background:#0095FF; color:white;', state.labelSelectKeys)
  }
