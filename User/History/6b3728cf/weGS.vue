<template>
  <div class="flex w-full base-types">
    ...
    <basic-modal @register="modalRegister" style="width: 40%">
      <select-bind-tag @handle-update-value="handleTreeUpdateValue" :selectKeys="state.labelSelectKeys" :data-type="bindType" :key="bindType" @close-modal="closeModal" />
    </basic-modal>
...
<script lang="ts" setup>
  ...
  // 选择违法行为类型
  function bindCauseType(record, type) {
    state.departmentTypeId = record.id
    state.bindType = type
    type === LawEnum.TAG ? getBindLawLabel(record.id) : getBindCauseLabel(record.id)
    openModal()
