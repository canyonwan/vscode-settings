    ...
    <n-grid cols="1 s:1 m:1 l:4 xl:4 2xl:4" responsive="screen" :x-gap="12">
      ...
      <n-grid-item span="2">
        <n-card title="违法行为类型" :segmented="{ content: 'soft' }">
          <law-tree-comp
            :law-data="causeTags"
            :request-type="LawEnum.CAUSETYPE"
            @on-handler="onCauseTagsHandler"
            :deleteAction="deleteCauseTag"
            :submitAction="saveCauseTags"
            :frontEndIdentification="['b08_01_01', 'b08_01_02']"
...
<script lang="ts" setup>
  ...
  import { deleteCauseTag, queryAllCauseTags, queryCauseTypes, saveCauseTags } from '@/api/cause-manage/cause-manage'
  ...
  // 获取违法行为标类型
  async function getCauseType() {
    await queryCauseTypes()
  }
