<template>
  <div class="target-info">
    <basic-form :schemas="schemasSearch" @register="register" @submit="queryData">
      <template #targetType>
      ...
      <template #tableTitle v-if="!isSelection">
    ...
    <div class="flex justify-end items-end mt-2" v-if="isSelection">
      ...
      <n-button class="ml-2" @click="selectSubmit" type="primary">提交</n-button>
    </div>
    <basic-modal @register="modalRegister" style="width: 70%" @on-ok="okModal" :title="typeName + modelTitle">
      <basic-form @register="registerForm" @submit="handleSubmit">
        <template #customForm="{ model }">
          <custom-form-use :type="3" ref="customFomRef" :targetType="typeId" :customFormValues="model.customFormValues" />
        </template>
        <template #CustomItem="{ model }">
          <custom-item v-model:customColumnsData="model.customColumnsData" />
        </template>
      </basic-form>
    </basic-modal>
  </div>
...
<script lang="ts">
  ...
  import { queryTargetData, saveTargetDetail, deleteTarget, queryTargetDetail } from '@/api/target/target-manage'
  ...
  export default defineComponent({
    ...
    emits: ['selectTargets', 'closeModel'],
    setup(props, context) {
      ...
      const handleAdd = () => {
        ...
        openModal()
        nextTick(() => {
          setFieldsValue({ ...state.formParam, typeId: state.typeId ? state.typeId : 0 })
        })
      }
      const okModal = () => {
        closeModal()
      ...
      const handleSubmit = async (res) => {
        res.customList = res.customColumnsData.map((item: any) => {
          if (item.fileInfoList && item.fileInfoList.length) {
            item.fileList = item.fileInfoList.map((listChild) => listChild.fileId).join(',')
          }
          return item
        })
        if (res.fileInfoList && res.fileInfoList.length) {
          res.fileList = res.fileInfoList.map((listChild) => listChild.fileId).join(',')
        }
        res.customFormValues = await customFomRef.value.saveCustomForm()
        await saveTargetDetail(res)
        closeModal()
        setFieldsValue({ ...state.formParam })
        reloadTable()
      }
      const handleEdit = async (record) => {
        state.modelTitle = '编辑'
        const data = await queryTargetDetail(record.id)
        openModal()
        nextTick(() => {
          setFieldsValue(data)
        })
      ...
      const selectSubmit = () => {
        if (checkedKeys.value.length) {
          context.emit('selectTargets', checkedKeys.value)
        } else {
