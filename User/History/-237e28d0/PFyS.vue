<script setup lang="ts">
  ...
  const stageBarBus = useEventBus('stageBarBusKey')

  stageBarBus.on((stage: number) => {
    stage ? filterTimelineNodes(stage) : getTimelineList()
  })

  const bus = useEventBus('wasteDispatch')
  bus.on(() => {
    console.log('%c [ wasteDispatch bus ]-30', 'font-size:13px; background:#0095FF; color:white;')
    getTimelineList()
  })
