  ...
  <div class="org-add-edit">
    <basic-form @close-modal="closeModal" @register="register" @submit="handleSubmit">
      <template #customForm="{ model, disabled }">
        <custom-form-use :type="1" :org-id="orgId" ref="customFomRef" :customFormValues="model.customFormValues" :disabled="disabled" />
      </template>
    </basic-form>
  </div>
</template>
  ...
  export default defineComponent({
    ...
    setup(props, context) {
      ...
      const handleSubmit = async (res) => {
        ...
        console.log('%c [ res ]-125', 'font-size:13px; background:#0095FF; color:white;', res)
        debugger
        if (props.isAdd) {
          const params = {
        ...
        addEditOrg({ ...info, ...res.customFormValues }).then((data) => {
