<script setup lang="ts">
  ...
  const props = defineProps<{ children: ITimeLineTreeLeaf[] }>()
</script>
<template>
  <div class="flex items-center" v-for="child in children" :key="child.id">
    <!-- 未开始 -->
    <n-icon v-if="child.nodeStatus === 0" class="text-gray-600" :component="EllipseOutline" />
    <!-- 进行中 -->
    ...
    <!-- 连接线 -->
    <div class="dashed flex-grow mr-2" v-if="itemNode?.childrenList && idx !== parentChildrenLength! - 1"></div>
