  ...
  setup(props, { slots }) {
    ...
    return () => (
      <>
          ...
          {state.groupList.map((item) => (
            <NSpace justify="space-between">
              <div>
                ...
                <NEllipsis style="max-width: 500px" class={props.useCheckbox && 'ml-2'} text type="primary" key={item.id} v-slots={{ default: () => item.name }} onClick={() => viewMenu(item.id!)}>
                  {item.name}
                </NEllipsis>
              </div>
        ...
        <NPagination class="mt-6" v-model:page={state.searchParams.page} v-model:page-size={state.searchParams.size} v-model:item-count={state.total} onUpdate:page={switchPage} />
        {slots.extraButton ? (
