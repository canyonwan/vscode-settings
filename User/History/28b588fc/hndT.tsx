...
import { discretionGroupSwitchEventKey } from '@/views/cause-library/manage/components/fooKey'
...
export default defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const viewMenu = (id: string) => {
      state.currentGroupItem.id = id
      bus.emit(['menu', state.currentGroupItem.id])
      // bus.emit(['menu', id])
    }
      ...
      <div>
        ...
        <NCheckboxGroup v-model:value={state.checkedGroups} onUpdateValue={onUpdateValue}>
          {state.groupList.map((item) => (
            <NSpace justify="space-between" class="hover:bg-mainHeader-gradientFrom cursor-pointer pl-1 rounded-sm" onClick={() => viewMenu(item.id!)}>
              <div>
                {props.useCheckbox && <NCheckbox value={item.id} />}
                <NEllipsis class={[props.useCheckbox && 'ml-2']} text type="primary" key={item.id} v-slots={{ default: () => item.name }}>
                  {item.name}
                </NEllipsis>
              </div>

              <NButtonGroup>
                {buttonShowPermission(['b01_06_01']) ? (
                  <NButton class="mr-2" type="success" secondary size="tiny" onClick={(evt) => editGroup(evt, item)} disabled={buttonDisabledPermission(['b01_06_01'])}>
                    编辑
                  </NButton>
                ) : null}
                {buttonShowPermission(['b01_06_02']) ? (
                  <NButton type="error" size="tiny" secondary onClick={(evt) => deleteGroup(evt, item.id!)} disabled={buttonDisabledPermission(['b01_06_02'])}>
                    删除
                  </NButton>
                ) : null}
              </NButtonGroup>
        ...
        <BasicModal onRegister={modalRegister} style="width: 40%">
          <SaveGroup currentGroupItem={state.currentGroupItem} onSubmitHandler={submitHandler} onCancelHandler={() => closeModal()} />
        </BasicModal>
        <NPagination
          class="mt-6"
          showSizePicker
        ...
        {slots.extraButton ? (
          ...
          <NButton class="mt-6" type="success" onClick={addDiscretionGroup} disabled={buttonDisabledPermission(['b01_06_01'])}>
            新增裁量分组
          </NButton>
        ) : null}
      </div>
