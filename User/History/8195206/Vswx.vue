<script setup lang="ts">
  ...
  import { options, createColumns, type ITaskSources } from './relate-task-options'
  ...
  const taskSources = ref<ITaskSources[]>([])
  let sponsorOptions = ref<any[]>([])
  ...
  function getTaskStageEnums() {
    sponsorOptions = handleEnumsKeys(['TaskStatus'])
  }

  async function getEnums() {
    taskSources.value = await queryEnums({ columnNameList: ['TaskType'] })
  }
...
<template>
  <div class="relate-task">
    <n-space>
      <n-select style="width: 180px" v-model:value="searchParams.taskStage" :options="options" />
