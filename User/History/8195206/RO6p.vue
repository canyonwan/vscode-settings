...
<template>
  <div class="relate-task">
    <n-space>
      <n-select style="width: 140px" v-model:value="searchParams.taskStage" :options="sources.taskStageEnums.slice(0, 4)" />
    ...
    <BasicTable :request="getTaskList" ref="tableRef :row-key="(row: ITaskItem) => row.id" :columns="createColumns({ ...sources })" :action-column="null" @update:checked-row-keys="updateCheckRowKeys"
    />
