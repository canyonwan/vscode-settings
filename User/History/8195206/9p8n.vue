<script setup lang="ts">
  ...
  async function onRelateTask() {
    let params: IRelateProgramOrTask = {
      ...
      dataId: props.caseInfo.procedureId,
      dataType: props.caseInfo.dataType,
      targetType: props.caseInfo.targetType,
...
<template>
  <div class="relate-task">
    <n-space>
      <n-select style="width: 140px" v-model:value="searchParams.taskStage" :options="sources.taskStageEnums.length === 6 ? sources.taskStageEnums.slice(0, 4) : sources.taskStageEnums" />
      <n-select style="width: 140px" v-model:value="searchParams.sponsor" :options="sponsorOptions" disabled />
