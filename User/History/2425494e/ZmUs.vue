<script setup lang="ts">
  ...
  function selectCaseType(row: IUnlawfulActItem, way: ICaseType) {
    const { setupData: data, selectedWays: s } = useSelectCaseTypes(row, way, setupData.value, selectedWays.value)
    setupData.value = data
    selectedWays.value = s
    console.log('%c [ selectedWays.value ]-42', 'font-size:13px; background:#0095FF; color:white;', selectedWays.value)
    console.log('%c [ setupData.value ]-40', 'font-size:13px; background:#0095FF; color:white;', setupData.value)
  }
