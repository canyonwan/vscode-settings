<script setup lang="ts">
  ...
  function renderPrefix({ option }) {
    if (option.list && option.list.length > 0) {
      return h(NEllipsis, { style: 'max-width: 200px;font-weight: bold' }, { default: () => `${option.name}` })
    } else {
      return h('div', { class: 'flex py-1' }, [
        // TODO 待后端补全图片字段
        h('img', { src: option?.fileList[0]?.thumbnailFileUrl || 'https://dummyimage.com/80x80/000/fff&text=TEST+IMAGE', class: ['mr-2', 'w-16'] }),
        h(
          NSpace,
          { vertical: true, justify: 'space-evenly' },
          {
            default: () => [
              ...
              h(NEllipsis, { style: 'max-width: 140px' }, { default: () => option.name })
            ]
          }
        )
      ])
