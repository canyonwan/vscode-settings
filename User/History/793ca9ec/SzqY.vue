...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    emits: ['onAddRow', 'onEditRow', 'onDeleteRow', 'onAddTopLevel', 'onCheckedKeysHandler', 'onCancelCheckKeys'],
    setup(props, { emit }) {
      ...
      function onAddRow(option: ICaseType) {
        console.log('新增子级 option', option)
        const params = {
          id: null,
          parentId: option.id
        }
        emit('onAddRow', option)
      }

      // 编辑行
      function onEditRow(option: ICaseType) {
        emit('onEditRow', option)
      }
