<script setup lang="ts">
  ...
  watch(
    ...
    () => {
      if (displayState.boundCauseTags.length > 0) tagFormInstRef.value?.restoreValidation()

      if (displayState.boundCauseLaws.length > 0) lawFormInstRef.value?.restoreValidation()

      if (displayState.boundDept.length > 0) deptFormInstRef.value?.restoreValidation()
    }
  )
...
<template>
  <div class="save-cause">
    <basic-form @register="register" @submit="onSubmit" @close-modal="() => $emit('onCloseModal')">
      <template #tag>
        <n-form-item ref="tagFormInstRef" label="案由标签" path="displayState.boundCauseTags" :rule="tagRule">
          <n-space vertical>
      ...
      <template #causeLaw>
        <n-form-item ref="lawFormInstRef" label="案由法" path="displayState.boundCauseLaws" :rule="lawRule">
          <n-space vertical>
