    ...
    <basic-form @register="register" @submit="onSubmit" @close-modal="() => $emit('onCloseModal')">
      <template #tag>
        <n-form-item ref="tagFormInstRef" label="违法行为标签" path="displayState.boundCauseTags" :rule="tagRule">
          <n-space vertical>
            <n-button ghost type="primary" @click="onSelectCauseTags">选择违法行为标签</n-button>
            <n-space>
      ...
      <template #causeLaw>
        <n-form-item ref="lawFormInstRef" label="违法行为法" path="displayState.boundCauseLaws" :rule="lawRule">
          <n-space vertical>
            <n-button ghost type="primary" @click="onSelectCauseLaw"
              ><template #icon> <n-icon :component="Add" /> </template>选择违法行为法</n-button
            >
            <n-space>
