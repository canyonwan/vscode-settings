<script setup lang="ts">
  ...
  watch(
    () => displayState.boundCauseLaws.length,
    () => {
      if (displayState.boundCauseLaws.length > 0) {
        console.log('%c [ displayState.boundCauseLaws ]-182', 'font-size:13px; background:#0095FF; color:white;', displayState.boundCauseLaws)
...
<template>
  <div class="save-cause">
    <basic-form @register="register" @submit="onSubmit" @close-modal="() => $emit('onCloseModal')">
      <template #tag>
        <n-form-item label="案由标签" path="displayState.boundCauseTags" :rule="tagRule">
          <n-space vertical>
      ...
      <template #causeLaw>
        <n-form-item ref="formInstRef" label="案由法" path="displayState.boundCauseLaws" :rule="lawRule">
          <n-space vertical>
      ...
      <template #dept>
        <n-form-item label="部门类型" path="displayState.boundDept" :rule="deptRule">
          <n-space vertical>
