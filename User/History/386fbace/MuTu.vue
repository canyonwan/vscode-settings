<script setup lang="ts">
  import { sendAnnouncement } from '@/api/infrom'
  import type { IAnnouncementSend, IMessageItem } from '@/api/infrom/types'
  ...
  let formValue = reactive<Partial<IAnnouncementSend>>({})
  ...
  watch(
    () => props.queryType,
    () => {
      if (props.queryType === 3 && props.rowData) {
        formValue.title = props.rowData.title
        formValue.content = props.rowData.content
        formValue.contacts = props.rowData.contacts
        formValue.contactNumber = props.rowData.contactNumber
      }
    }
  )
</script>
