<script setup lang="ts">
  ...
  async function onSubmit(type: number) {
    formRef.value?.validate(async (error) => {
      ...
      if (error && state.selectedOrgKeys.length > 0) {
        ...
        if (props.announcementId) params.id = props.announcementId
  ...
  async function getAnnouncementDetail() {
    ...
    formValue.content = res.content
    formValue.contactNumber = res.contactNumber
    formValue.contacts = res.contacts
    formValue.receivingGroupTypeNames = res.receivingGroupTypeNames
    state.selectedOrgKeys = res.receivingGroupTypeIds.split(',')
    state.fileInfos = res.fileInfos! // 回显展示
    state.fileIds = res.fileInfos?.map((file) => file.fileId).join(',') || ''
  }
    ...
    <n-form inline ref="formRef" label-placement="left" label-width="auto" :model="formValue" :rules="rules">
      <n-grid :cols="24" :x-gap="24">
        ...
        <n-form-item-gi :span="12">
          <label class="ml-6 n-form-item-label">文件<span class="n-form-item-label__asterisk">&nbsp;*</span></label>
          <basic-upload @update:value="handleUploaded" :value="state.fileInfos" />
        </n-form-item-gi>
      </n-grid>
    </n-form>
