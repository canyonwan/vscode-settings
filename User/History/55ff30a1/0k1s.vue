<script setup lang="ts">
  ...
  function handleItemInfo(itemInfo: IItemNodeProps) {
    ...
    // console.log('handleItemInfo', itemInfo.leftDividerWidth)
    calcWidth()
  }

  function calcWidth() {
    nodeEl.value?.dispatchItemWidth()
    eligibleIds.value.length = 0
    if (timelineNodes?.value?.length) {
      eligibleIds.value.push(timelineNodes.value!.at(0)!.id)
    }
    let w = 0
    console.log('left width', nodeEl.value?.leftDividerWidth));
    // let currentContainerWidth = containerWidth.value - nodeEl.value?.leftDividerWidth.value
    ...
    console.log('eligibleIds', eligibleIds.value)
  }
