...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const createActions = (record: any) => {
      return [
        ...
        },
        {
          type: 'success',
          label: '新增子清单',
          onClick: addSecondaryMenu.bind(null, record)
        }
      ]
    }
    // 分页获取所有清单
    ...
    const editMenu = (record: IEvidenceMenuType) => {
      const { id, name, whetherRequire, whetherGroup, groupMiniRequire, tips, relaEvidenceType, remarks, provMatters } = record
      setProps({ title: '编辑清单' })
      openModal()
      onSwitchComp(14)
    ...
    // 新增证据清单
    const addEvidenceMenu = () => {
      state.evidenceMenuItem.id = ''
      setProps({ title: '新增清单' })
      openModal()
      onSwitchComp(14)
    }
    ...
    // 新增子清单
    const addSecondaryMenu = (record: IEvidenceMenuType) => {
      setProps({ title: '新增子清单' })
      openModal()
      onSwitchComp(14)
      state.evidenceMenuItem.addSecondaryMenu = true
      state.evidenceMenuItem.id = record.id
    }
    ...
    watchEffect(() => {
      if (props.useSelection) {
        tableColumns.at(0)!.type = 'selection'
        state.checkedKeys = props.boundKeys || []
      } else {
        // tableColumns.at(0)!.type = ''
      }
    })
    ...
    return () => (
      <div>
        <NCard bordered={false}>
          ...
          >
            {{
              ...
              toolbar: () => (
                <NButton type="success" onClick={addEvidenceMenu}>
                  新增证据清单
                </NButton>
              )
            }}
        ...
        <BasicModal onRegister={modalRegister} style={{ width: '50%' }}>
          {h(currentComp.value, { onOkSaveEvidenceMenu: okSaveEvidenceMenuHandler, evidenceMenuItem: state.evidenceMenuItem })}
        </BasicModal>
      </div>
    )
  }
