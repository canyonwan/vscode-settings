...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const state = reactive({
      ...
      noticeTimeR: [] as any as TimeRangeValueType,
      askCountData: [],
      noticeCountData: [],
      askMe: false, // 是否我发送
    ...
    const noticeChange = () => {
      ...
      if (state.noticeTimeR) {
        state.searchData.beginTime = dayjs(state.noticeTimeR[0]).format('YYYY-MM-DD 00:00:00')
        state.searchData.endTime = dayjs(state.noticeTimeR[1]).format('YYYY-MM-DD 23:59:59')
      } else {
        state.searchData.beginTime = undefined
        state.searchData.endTime = undefined
      }
