...
export default defineComponent({
  ...
  setup(props) {
    ...
    const state = reactive({
      timeList: [] as TimeRangeValueType,
      dataSource: [] as any,
      loading: false,
      pagination: {
    ...
    const getData = () => {
      ...
      if (state.timeList && state.timeList.length > 0) {
        params.sentTimeStart = dayjs(state.timeList[0]).format('YYYY-MM-DD 00:00:00')
        params.sentTimeEnd = dayjs(state.timeList[1]).format('YYYY-MM-DD 23:59:59')
      } else {
