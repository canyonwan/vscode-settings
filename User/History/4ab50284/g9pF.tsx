import { Ref } from 'vue'
import { NButton, NForm, NFormItem, NInputNumber, NRadio, NRadioGroup, NSpace, NTag, useMessage } from 'naive-ui'
...
export default defineComponent({
  ...
  setup(props: HandlingWayCommonPropsType, { emit }) {
    ...
    const rule = {
      minValue: {
        trigger: ['input', 'blur'],
        state.caseTypes.some((item) => item.id === '1000034' && validator() {
          if (!state.minValue) {
    ...
    return () => {
      return (
        <>
          <NForm labelPlacement="left" labelWidth="80px" model={state} rules={rule}>
            ...
            {state.caseTypes.some((item) => item.id === '1000034') && (
              <>
                {/* 是简易给1000000 不是给空 */}
                <NFormItem label="是否简易">
                  <NRadioGroup v-model:value={state.currentSimpleProgram}>
                    {simpleProgramSources.map((item) => (
                      <NRadio value={item.value} label={item.label}>
                        {item.label}
                      </NRadio>
                    ))}
                  </NRadioGroup>
