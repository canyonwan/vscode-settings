<script setup lang="ts">
  ...
  function renderSuffix({ option }: { option: TreeOption | ICauseType }) {
    return [
      h(NButton, { text: true, type: 'success', class: 'mr-2', onClick: () => onEdit(option) }, { default: () => '编辑' }),
...
<template>
  <div class="cause-type">
    ...
    <BasicModal @register="register">
      <save-cause-type :cause-id="causeId" @submit-handler="submitHandler" />
    </BasicModal>
  </div>
</template>
