...
<script setup lang="ts">
  ...
  const handleAddSubItem = async (row: IDiscretionMenuType) => {
    ...
    state.currentRowData.id = row.id
...
<template>
  <div>
    ...
    <basic-modal @register="modalRegister" style="width: 70%">
      <component :is="currentComp" :current-row-data="state.currentRowData" @submit-handler="submitHandler" @cancel-modal-handler="cancelModalHandler" />
    </basic-modal>
  </div>
</template>
