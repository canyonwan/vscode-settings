...
<script setup lang="ts">
  ...
  const createActions = (record: IDiscretionMenuType) => {
    return [
      {
        frontEndIdentification: ['b01_04_01'],
        type: 'info',
        label: '编辑',
        onClick: handleEdit.bind(null, record)
      },
      {
        frontEndIdentification: ['b01_04_02'],
        type: 'error',
        label: '删除',
  ...
  const handleDelete = async (row: IDiscretionMenuType) => {
    dialog.error({
      title: '提示',
      content: `${row.children && row.children.length > 0 ? '该裁量清单下包含子清单，你确定一并删除吗？' : '你确定要删除吗？'}`,
      positiveText: '确定',
      negativeText: '取消',
  ...
  <div>
    <n-card>
      <basic-table
        ...
        :frontEndIdentification="['b01_04_01']"
      >
        <template #toolbar="{ disabled }" v-if="!slots.toolbar && !props.useSelection">
          <n-button type="primary" @click="addMenu" :disabled="disabled"> 新增裁量清单 </n-button>
        </template>
