<script setup lang="ts">
  ...
  const schemasForm = ref(punishSchemas)
  ...
  interface ISearchParams {
    ...
    docCode: string // 文书号
    causeTypeId: string // 违法行为型
    caseTypeId: string[] // 案件类型
  }
  ...
  function onSearch(values: ISearchParams) {
    console.log('%c [ values ]-128', 'font-size:13px; background:#0095FF; color:white;', values)
    const userIds = hasSelectedState.selectedPersoneel.map((item) => item.id!)
    searchParams = { ...values, userId: userIds, hostOrganId: hasSelectedState.selectedOrgKeys, procedureType: props.programType }
    reloadTable({ page: 1 })
  }
