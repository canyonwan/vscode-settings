<script setup lang="ts">
  ...
  onMounted(() => {
    console.log('evidence', injectNodeRubbish?.value)
  })
</script>
<template>
  <n-checkbox-group @update:value="onUpdateValue" :value="injectCheckedChildren">
    <n-space vertical>
      <n-checkbox v-for="node in injectNodeRubbish" :key="node.evidenceId" :value="node.evidenceId">
        <n-card>
          <div class="font-medium text-base">{{ node.evidenceName }}</div>
          <!-- media -->
          <div class="py-3">
