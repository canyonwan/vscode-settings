  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      interface IState {
        ...
        checkedRaw: TreeOptions[] // 已选中的原数据
      }
      ...
      function updateCheckedKeys(values: string[], rows: ILawTagType[]) {
        console.log('%c [ rows ]-235', 'font-size:13px; background:#0095FF; color:white;', rows)
        ...
        let child = {}
        console.log('%c [ res ]-233', 'font-size:13px; background:#0095FF; color:white;', res)
        for (let obj in res) {
          for (const arr of res[obj]) {
            children.push(arr)
          }
        }

        // if (children.length > 1) {
        //   child = children.splice(-1)
        // } else {
        //   child = children[0]
        // }
        console.log('%c [ children ]-241', 'font-size:13px; background:#0095FF; color:white;', children)
        // const child = childArr.reduce((pre: ILawTagType[], cur) => {
        //   const has = pre.filter((item) => item.parentId === cur.parentId)
        //   if (has && has.length > 0) {
        //     return pre.push(...has)
        //   }
        //   return cur

        //   if (pre.parentId === cur.parentId) {
        //     return cur
        //   } else {
        //     return pre
        //   }
        // }, [])
        // console.log({ child, parentArr, childArr })
        // let idx = -1
        // setTimeout(() => {
        //   idx = childArr.findIndex((item) => item.id === child.id)
        // }, 100)
        // console.log({ idx, child })
        // idx > 0 && childArr.splice(idx, 1)
        state.checkedAllKeys = [...parentArr.map((item) => item.id!), child && child!.id]
