<template>
  <div class="law-tag">
    ...
    <n-tree
      ...
      :virtual-scroll="lawTags.length > 100 ? true : false"
      :style="{ height: lawTags.length > 100 ? '400px' : null }"
    />
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      function renderLabel({ option }: { option: TreeOption }) {
        return [h(NEllipsis, { style: 'max-width: 600px' }, { default: () => option.name })]
      }

      function renderPrefix({ option }) {
        return h(NEllipsis, { style: 'max-width: 600px' }, { default: () => `${option.name}${option.code ? `-${option.code}` : ''}` })
      ...
      function updateCheckedKeys(values: string[], rows: ILawTagType[]) {
        if (props.useRadio) {
          const { checkedAllKeys, checkedRaw } = useTreeRadio(rows)
          state.checkedAllKeys = checkedAllKeys.value
          state.checkedRaw = checkedRaw.value
        } else {
