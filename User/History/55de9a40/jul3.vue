...
export default defineComponent({
  ...
  setup() {
    ...
    const selectMenu = (select: any) => {
      if (select.key) {
        console.log('%c [ select ]-74', 'font-size:13px; background:#0095FF; color:white;', select)
        if (select.key === 'publicPlatformBefore') {
          window.open('/publicityPlatform.html/before')
        }
