...
export function queryTaskBaseInfo(taskId: string): Promise<ITaskBaseInfo> {
  return http.request({
    url: `${ServerName}timeline/task/taskInfo?taskId=${taskId}`,
    method: RequestEnum.POST
  })
}
 ...
 * @param params
 */
export function queryDataPackage(programId: string): Promise<ITaskTimelineInfo> {
  return http.request({
    url: `${ServerName}procedures/informationPackage/${programId}`,
