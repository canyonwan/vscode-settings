...
export const createColumns = ({ sendMail, allowSelect }: { sendMail: (ids: string[], parentId: string) => void; allowSelect: boolean }): DataTableColumns<RowData> => {
  return [
    {
      title: '案由编号',
      key: 'identifier',
            ...
            {},
            {
              default: () => {
                return [
                  ...
                  h(
                    NCheckboxGroup,
                    { onUpdateValue: (values: string[]) => sendMail(values, ele.id) },
                    { default: () => ele.list.map((e) => h(NCheckbox, { label: `${e.name}+${e.id}`, value: e.id })) }
                  )
                ]
              }
            }
