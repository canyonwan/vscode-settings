...
export const createColumns = ({ sendMail, allowSelect, ids }: { sendMail: (values: string[], pId: string) => void; allowSelect: boolean; ids: string[] }): DataTableColumns<RowData> => {
  return [
    {
      title: '案由编号',
      key: 'identifier',
            ...
            {},
            {
              default: () => {
                return [
                  ...
                  h(
                    NCheckboxGroup,
                    { onUpdateValue: (values: string[]) => sendMail(values, ele.id), value: ids },
                    { default: () => ele.list.map((e) => h(NCheckbox, { label: e.name, value: `${row.identifier}-${ele.id}-${e.id}` })) }
                  )
                ]
              }
            }
