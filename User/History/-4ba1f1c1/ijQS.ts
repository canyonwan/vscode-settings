import type { IEvidenceMenuType, IUnlawfulActItem } from '@/api/cause-manage/types'
import { DataTableColumn, DataTableColumns, NCheckbox, NCheckboxGroup, NSpace } from 'naive-ui'
import { RowData } from 'naive-ui/lib/data-table/src/interface'

export const createColumns = ({ sendMail, allowSelect }: { sendMail: (rowData: string[]) => void; allowSelect: boolean }): DataTableColumns<RowData> => {
  return [
    {
      title: '案由编号',
      key: 'identifier',
    ...
    },
    {
      ...
      render(row: IUnlawfulActItem) {
          ...
          return h(
            ...
            {},
            {
              default: () => {
                return [
                  `${ele.name}： `,
                  h(NCheckboxGroup, { onUpdateValue: (values: string[]) => sendMail(values) }, { default: () => ele.list.map((e) => h(NCheckbox, { label: e.name, value: e.id })) })
                ]
              }
            }
          )
        ...
        const b = row.caseTypeList?.map((ele, index) => `${ele.name}：${ele.list.map((minor) => minor.name)}${index === row.caseTypeList!.length - 1 ? '' : '、'}`)

        return allowSelect ? canSelectRows : b
      }
    },
    {
      title: '检查对象类型',
