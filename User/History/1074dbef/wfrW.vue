  ...
  <div class="credential-manage">
    ...
    <a-card class="data-table-card" hoverable>
      ...
      <a-table
        ...
        :row-selection="showCheckbox ? { selectedRowKeys: selectedRowKeys, onChange: onSelectChange } : null"
        :columns="columns"
  ...
  setup() {
    ...
    function switchSelections() {
      if (state.showCheckbox) {
        console.log('%c [ state.showCheckbox ]-543', 'font-size:13px; background:#0095FF; color:white;', state.showCheckbox)
        return { selectedRowKeys: state.selectedRowKeys, onChange: onSelectChange }
      }
      return null

      // showCheckbox ? { selectedRowKeys: selectedRowKeys, onChange: onSelectChange } : null
    }
    getEnumData()
    return {
      switchSelections,
