<script setup lang="ts">
  ...
  bus.on(async () => {
    if (calcMoneyState.checkedDiscretionDetail.length > 0 && calcMoneyState.causeDiscretionId) {
  ...
  onMounted(async () => {
    ...
    if (calcMoneyState.checkedDiscretionDetail.length > 0 && calcMoneyState.causeDiscretionId && calcMoneyState.baseValue > 0) {
      await useCalcMoney(calcMoneyState)
    }
  })
