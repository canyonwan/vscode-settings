<script setup lang="ts">
  ...
  const discretionBaseState = ref<IDiscretionBaseState>()

  const bus = useEventBus('dispatchDiscretionBaseStateKey')
  bus.on((data: IDiscretionBaseState) => {
    console.log('%c [menu bus data ]-27', 'font-size:13px; background:pink; color:#bf2c9f;', data)
    discretionBaseState.value = data
  })
  ...
  const calcMoneyState = reactive<ICalcDiscretionSumFormalParameter>({
    ...
    causeDiscretionId: discretionBaseState.value?.discretionBaseId, // 裁量类型id
    baseValue: discretionBaseState.value?.baseValue || 0, // 裁量基准值
