<script setup lang="ts">
  ...
  const discretionBaseStateLocal = ref<IDiscretionBaseState>()

  watch(
    () => props.discretionBaseState.baseValue,
    () => {
      discretionBaseStateLocal.value = props.discretionBaseState
      // discretionBaseStateLocal.value!.discretionBaseId = props.discretionBaseState.discretionBaseId
      console.log('%c [ whenever props.discretionBaseState ]-32', 'font-size:13px; background:pink; color:#bf2c9f;', props.discretionBaseState)
      console.log('%c [ whenever discretionBaseStateLocal ]-32', 'font-size:13px; background:pink; color:#bf2c9f;', discretionBaseStateLocal.value)
    }
  )
  ...
  const calcMoneyState = reactive<ICalcDiscretionSumFormalParameter>({
    ...
    causeDiscretionId: discretionBaseStateLocal.value?.discretionBaseId, // 裁量类型id
    baseValue: discretionBaseStateLocal.value?.baseValue || 0, // 裁量基准值
  ...
  onMounted(async () => {
    ...
    console.log('%c [ mounted props.discretionBaseState ]-74', 'font-size:13px; background:pink; color:#bf2c9f;', props.discretionBaseState)
