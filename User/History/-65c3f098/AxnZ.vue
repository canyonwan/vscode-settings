<script setup lang="ts">
  ...
  onBeforeMount(() => {
    console.log('%c [ props.discretionBaseState ]-32', 'font-size:13px; background:pink; color:#bf2c9f;', props.discretionBaseState)
  })

  watch(
    () => props.discretionBaseState.baseValue,
    () => {
      discretionBaseStateLocal.value!.baseValue = props.discretionBaseState.baseValue
  ...
  const calcMoneyState = reactive<ICalcDiscretionSumFormalParameter>({
    ...
    causeDiscretionId: props.discretionBaseState.discretionBaseId, // 裁量类型id
    baseValue: props.discretionBaseState.baseValue, // 裁量基准值
